...
void automap::draw() {
	...
	long sx, ex, sy, ey, x, y, window_xstart, window_ystart, window_xend,
			window_yend, centerx, centery, draw_xstart, draw_ystart, i, j;
	...
	if (sx < 0) // does the map scroll past the left side ?
		...
		draw_xstart = centerx
				- (x * AUTOTILE_WIDTH / f_wid - sx * AUTOTILE_WIDTH);
	...
	if (sy < 0) {
		...
		draw_ystart = centery
				- (y * AUTOTILE_HEIGHT / f_hi - sy * AUTOTILE_HEIGHT);
	...
	if (draw_xstart == old_dx && draw_ystart == old_dy) {
		...
		automap_window->screen->AddDirty(centerx, centery, centerx + 1,
				centery + 1);
		if ((tick++) & 4)
	...
	// everything dirty
	screen->AddDirty(window_xstart, window_ystart, window_xend + 1,
			window_yend + 1);
	...
	screen->SetClip(window_xstart, window_ystart, window_xend + 1,
			window_yend + 1);
	...
	for (j = draw_ystart, y = sy; y <= ey; j += AUTOTILE_HEIGHT,y++) {
		...
		for (i = draw_xstart, x = sx; x <= ex;
				i += AUTOTILE_WIDTH,x++, fgline++) {
			if ((*fgline) & 0x8000) {
				int id = foretiles[(*fgline) & 0x7fff];
				if (id >= 0)
					cache.foret(id)->micro_image->put_image(screen, i, j, 0);
				else
					cache.foret(foretiles[0])->micro_image->put_image(screen, i,
							j, 0);
			} else
				screen->bar(i, j, i + AUTOTILE_WIDTH - 1,
						j + AUTOTILE_HEIGHT - 1, 0);
		}
	}
