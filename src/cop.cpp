...
signed char small_fire_off[24 * 2] = // x & y offset from character to end of gun.
		{ 17,
				...
				7, 43, -3,
				44, // 7
				-10, 42, -16, 39, -20, 34, -20, 28, -20, 25, -19, 20, -19, 16,
				-16, 14, -14, 11, -11, 9, -7, 8, -3, 8, 2, 8, 6, 9, 10, 10, 14,
				13, 16, 15 };

signed char large_fire_off[24 * 2] = { 18, 25, 17, 30, 15, 34, 14, 36, 10, 39,
		7, 41, 4, 42, -3, 41, -8, 39, -11, 37, -14, 33, -16, 30, -18, 25, -17,
		21, -14, 17, -11, 15, -7, 13, -4, 12, 3, 12, 9, 12, 12, 15, 14, 16, 15,
		18, 16, 21 };
...
enum {
	NO_POWER, FAST_POWER, FLY_POWER, SNEAKY_POWER, HEALTH_POWER
};

enum {
	top_point_angle, top_fire_delay1, top_just_fired
};
		...
		if (v) {
			if (!v->freeze_time) {
				...
				signed char *fire_off =
						o->otype == S_DFRIS_TOP ?
								large_fire_off :
								(o->otype == S_ROCKET_TOP ?
										large_fire_off :
										(o->otype == S_BFG_TOP ?
												large_fire_off : small_fire_off));
				signed char *f = fire_off, *fb = NULL;
				int best_diff = 200, best_num = 0;
				int iy = f[1], ix = f[6 * 2];

				int best_angle = lisp_atan2(q->y - iy - v->pointer_y,
						v->pointer_x - q->x - ix);
				...
				if (abs(q->y - fb[1] - v->pointer_y) < 45
						&& abs(v->pointer_x - q->x + fb[0]) < 40)
					o->lvars[point_angle] = lisp_atan2(fb[1] - iy, fb[0] - ix);
				else
					o->lvars[point_angle] = lisp_atan2(
							q->y - fb[1] - v->pointer_y,
							v->pointer_x - (q->x + fb[0]));
...
static int player_fire_weapon(game_object *o, int type, game_object *target,
		int angle, signed char *fire_off) {
	...
	if (signal == l_FIRE) {
		if (!o->lvars[fire_delay1]) // make sur we are not waiting of previous fire
		{
			...
			if (value) // do we have ammo ?
			{
				...
				if (player_fire_weapon(o, SHOTGUN, NULL, o->lvars[point_angle],
						small_fire_off))
				...
				player_fire_weapon(o, SHOTGUN, NULL, o->lvars[point_angle],
						small_fire_off);
				ret = LNumber::Create(0);
			}
		} else
	...
	if (signal == l_FIRE) {
		if (!o->lvars[fire_delay1]) // make sur we are not waiting of previous fire
		{
			...
			if (value) // do we have ammo ?
			{
				...
				if (player_fire_weapon(o, ammo_type(o->otype), NULL,
						o->lvars[point_angle],
	...
	if (signal == l_FIRE) {
		if (!o->lvars[fire_delay1]) // make sur we are not waiting of previous fire
		{
			...
			if (value) // do we have ammo ?
			{
				...
				if (player_fire_weapon(o, PLASMA, NULL, o->lvars[point_angle],
						small_fire_off))
					ret = LNumber::Create(-1);
				else
					ret = LNumber::Create(0);
	...
	if (signal == l_FIRE) {
		if (!o->lvars[fire_delay1]) // make sur we are not waiting of previous fire
		{
			...
			if (value) // do we have ammo ?
			{
				...
				if (player_fire_weapon(o, LSABER, NULL,
						o->lvars[point_angle]
								+ (current_level->tick_counter() & 7) - 8,
						small_fire_off))
					ret = LNumber::Create(-1);
				else
					ret = LNumber::Create(0);
...
void *player_rocket_ufun(void *args) {
	...
	if (signal == l_FIRE) {
		if (!o->lvars[fire_delay1]) // make sur we are not waiting of previous fire
		{
			...
			{
				o->lvars[fire_delay1] = 6;
				game_object *target = NULL, *p, *bot =
						o->total_objects() ? o->get_object(0) : 0;
				if (bad_guy_array) {
					game_object *other =
							current_object->total_objects() ?
									current_object->get_object(0) : 0;
					for (p = current_level->first_active_object(); p;
							p = p->next_active) {
						xd = abs(p->x - o->x);
						yd = abs(p->y - o->y);
						if (xd < 160 && yd < 130 && bad_guy_array[p->otype]
								&& p != other) {
							if (p->targetable()
									&& !(p->otype == S_ROCKET
											&& p->total_objects()
				...
				if (player_fire_weapon(o, ROCKET, target, o->lvars[point_angle],
						large_fire_off))
					ret = LNumber::Create(-1);
				else
					ret = LNumber::Create(0);
...
static void do_special_power(game_object *o, int xm, int ym, int but,
		game_object *top) {
	switch (o->lvars[special_power]) {
	case FLY_POWER: {
		game_object *cloud = create(S_CLOUD, o->x + o->direction * -10,
				o->y + jrand() % 5);
		if (current_level)
...
void *cop_mover(int xm, int ym, int but) {
	...
	if (o->controller() && o->controller()->freeze_time) {
		...
		if (but || o->controller()->key_down(JK_SPACE)
			...
			if (o->hp() == 0) {
				...
				if (o->hp() < 40 && (current_level->tick_counter() % 16) == 0) // if low on health play heart beat
					the_game->play_sound(S_LOW_HEALTH_SND, 127, o->x, o->y);
						...
						&& (current_level->tick_counter() % 8) == 0) // if low on health play heart beat
					the_game->play_sound(S_LOW_HEALTH_SND, 127, o->x, o->y);
				else if (o->hp() < 7
						&& (current_level->tick_counter() % 4) == 0) // if low on health play heart beat
					the_game->play_sound(S_LOW_HEALTH_SND, 127, o->x, o->y);
				...
				if ((but & 2) && !o->lvars[is_teleporting]
						&& o->state != S_climbing && o->state != S_climb_off) {
					void *args = NULL;
					PtrRef r1(args);
					view *v = o->controller();

					push_onto_list(
							LNumber::Create(v->weapon_total(v->current_weapon)),
							args);
					push_onto_list(l_FIRE, args);

					current_object = top;
					void *ret = ((LSymbol *) figures[top->otype]->get_fun(
							OFUN_USER_FUN))->EvalFunction(args);
					current_object = o;
...
void *ladder_ai() {
	...
	if (o->total_objects()) {
		...
		for (; f; f = f->next) {
			...
			if (o->x <= mex && o->y <= mey && other->x >= mex
					&& other->y >= mey) {
...
void *player_draw(int just_fired_var, int num) {
	...
	if (num == 0) {
		...
		if (o->lvars[just_fired_var]) {
			o->draw_double_tint(
					lnumber_value(
							((LArray *) ((LSymbol *) l_player_tints)->GetValue())->Get(
									num)), S_bright_tint);
			o->lvars[just_fired_var] = 0;
		} else
			o->draw_tint(
	...
	if (o->aistate() > 0) {
		...
		case HEALTH_POWER: {
			...
			if (o->controller() && o->controller()->local_player())
				cache.img(S_health_image)->put_image(screen,
						o->controller()->cx2 - 20, o->controller()->cy1 + 5, 1);
		}
			break;
		case FAST_POWER: {
			...
			if (o->controller() && o->controller()->local_player())
				cache.img(S_fast_image)->put_image(screen,
						o->controller()->cx2 - 20, o->controller()->cy1 + 5, 1);
		}
			break;
		case FLY_POWER: {
			...
			if (o->controller() && o->controller()->local_player())
				cache.img(S_fly_image)->put_image(screen,
						o->controller()->cx2 - 20, o->controller()->cy1 + 5, 1);
		}
			break;
		case SNEAKY_POWER: {
			...
			if (o->controller() && o->controller()->local_player())
				cache.img(S_sneaky_image)->put_image(screen,
						o->controller()->cx2 - 20, o->controller()->cy1 + 5, 1);
		}
			break;
		}
...
void *sgun_ai() {
	...
	int whit = 0;
	game_object *who = o->bmove(whit,
			o->total_objects() ? o->get_object(0) : 0);

	if (whit
			|| (who && figures[who->otype]->get_cflag(CFLAG_UNACTIVE_SHIELD)
					&& who->total_objects()
					&& who->get_object(0)->aistate() == 0)) {
		o->lvars[sgb_lifetime] = 0;
		game_object *n = create(S_EXPLODE5, o->x + jrand() % 4,
				o->y + jrand() % 4);
		current_level->add_object(n);
	} else if (who && figures[who->otype]->get_cflag(CFLAG_HURTABLE)) {
		o->lvars[sgb_lifetime] = 0;
		game_object *n = create(S_EXPLODE3, o->x + jrand() % 4,
				o->y + jrand() % 4);
		current_level->add_object(n);
		who->do_damage(5, o, o->x, o->y, (lisp_cos(ang) * 10) >> 16,
				(lisp_sin(ang) * 10) >> 16);
	}
	return true_symbol;
}
...
void *score_draw() {
	...
	if (local) {
		...
		for (i = 0; i < tp; i++) {
			int color =
					lnumber_value(
							((LArray *) ((LSymbol *) l_player_text_color)->GetValue())->Get(
									sorted_players[i]->get_tint()));
			sprintf(msg, "%3ld %s", (long) sorted_players[i]->kills,
...
void *show_kills() {
	...
	screen->clear();
	image *im = cache.img(
			cache.reg("art/frame.spe", "end_level_screen", SPEC_IMAGE, 1));
	...
	screen->widget_bar(x, y + 2, x + strlen(header_str) * fnt->width(),
			y + fnt->height() - 3, wm->bright_color(), wm->medium_color(),
			wm->dark_color());
	y += fnt->height();
	v = player_list;
	for (i = 0; i < tp; i++) {
		...
		sprintf(msg, "%-17s %3ld  %3ld", max_name, (long) v->kills,
