...
void view::add_ammo(int weapon_type, int total) {
	...
	if (weapon_total(current_weapon) == 0 && current_weapon) {
		...
		if (DEFINEDP(symbol_value(l_switch_to_powerful))
				&& symbol_value(l_switch_to_powerful)) {
...
int32_t view::interpolated_xoff() {
	if (focus) {
		int x = (last_last_x + last_x) / 2 - (cx2 - cx1 + 1) / 2 + shift_right
				+ pan_x;
		if (x < 0)
			return 0;
		else
...
int32_t view::interpolated_yoff() {
	if (focus) {
		int y = (last_y + last_last_y) / 2 - (cy2 - cy1 + 1) / 2 - shift_down
				+ pan_y;
		if (y < 0)
			return 0;
		else
...
//        file "input.lsp" to get our key mappings.
	 ...
	 else*/
	{
		...
		if (focus) {
			the_game->mouse_to_game(last_demo_mx, last_demo_my, sug_px, sug_py,
					this);
			if (last_demo_mbut & 1)
				sug_b2 = 1;
			if (last_demo_mbut & 2)
...
void view::set_input(int cx, int cy, int b1, int b2, int b3, int b4, int px,
		int py) {
	x_suggestion = cx;
	y_suggestion = cy;
	b1_suggestion = b1;
...
void view::reset_player() {
	if (focus) {
		...
		game_object *start =
				current_level ?
						current_level->get_random_start(320,
								focus->controller()) :
						0;
		focus->defaults();
		if (start) {
		...
		if (figures[focus->otype]->get_fun(OFUN_CONSTRUCTOR)) {
			game_object *o = current_object;
			current_object = focus;
			((LSymbol *) figures[focus->otype]->get_fun(OFUN_CONSTRUCTOR))->EvalUserFunction(
					NULL);
			current_object = o;
		}
		sbar.redraw(screen);
...
static char const *vv_names[TVV] = { "view.cx1", "view.cy1", "view.cx2",
		"view.cy2", "view.shift_down", "view.shift_right", "view.god",
		"view.player_number", "view.draw_solid", "view.lives",
		"view.current_weapon", "view.x_suggestion", "view.y_suggestion",
		"view.b1_suggestion", "view.b2_suggestion", "view.b3_suggestion",
		"view.b4_suggestion", "view.pan_x", "view.pan_y", "view.no_xleft",
		"view.no_xright", "view.no_ytop", "view.no_ybottom", "view.last_x",
		"view.last_y", "view.last_left", "view.last_right", "view.last_up",
		"view.last_down", "view.last_b1", "view.last_b2", "view.last_b3",
		"view.last_b4", "view.last_hp", "view.secrets", "view.kills",
		"view.tsecrets", "view.tkills", "view.ambient", "view.pointer_x",
		"view.pointer_y", "view.last_last_x", "view.last_last_y",
		"view.freeze_time" };
