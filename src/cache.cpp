	...
	if (type < 0xffff) {
		if (!figures[type]->get_cflag(CFLAG_NEED_CACHE_IN)) // see if it's already marked
			...
			if (cache_fun) {
				...
				void *CacheList = ((LSymbol *) cache_fun)->EvalFunction(
						call_with);
				PtrRef r1(CacheList);
				...
				if (CacheList && lcdr(CacheList)) {
					...
					while (id_list) {
						...
						if (id < 0 || id >= total)
							lbreak(
									"Get cache list returned a bad id number %d\n",
									id);
...
void CacheList::free_oldest() {
	...
	if (oldest) {
		...
		close_graphics();
		printf(
				"Out of memory, please remove any TSR's device drivers you can\n");
		exit(0);
	}
}
...
void CacheList::show_accessed() {
	...
	do {
		...
		ci = list;
		for (int i = 0; i < total; i++, ci++) {
			if (ci->last_access < old && ci->last_access > 0
					&& ci->last_access > new_old_accessed) {
