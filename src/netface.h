...
// client commands
enum {
	CLCMD_JOIN_FAILED, CLCMD_JOIN_SUCCESS, CLCMD_RELOAD_START, // will you please load netstart.spe
...
enum {
	NF_OPEN_FAILED, NF_OPEN_LOCAL_FILE, // should return path to local file as well
	NF_OPEN_REMOTE_FILE
};
...
struct net_packet {
	...
	void add_to_packet(void *buf, int size) {
		if (size
				&& size + packet_size() + packet_prefix_size() < PACKET_MAX_SIZE) {
