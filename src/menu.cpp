...
//percent is 0..256
void tint_area(int x1, int y1, int x2, int y2, int r_to, int g_to, int b_to,
		int percent) {
	int x, y;
	int cx1, cy1, cx2, cy2;
...
int menu(void *args, JCFont *font) // reurns -1 on esc
	...
	int mx = screen->Size().x / 2 - mw / 2, my = screen->Size().y / 2 - mh / 2;
	...
	if (title) {
		...
		dark_widget(tx - 2, my - font->height() - 4, tx + tl + 2, my - 2,
				wm->medium_color(), wm->dark_color(), 180);
		font->put_string(screen, tx + 1, my - font->height() - 2, title,
				wm->bright_color());
	}

	dark_widget(mx, my, mx + mw - 1, my + mh - 1, wm->medium_color(),
			wm->dark_color(), 200);
	...
	do {
		...
		if (wm->event_waiting()) {
			...
			if (ev.type == EV_KEY) {
				...
				if (ev.mouse_move.x > mx && ev.mouse_move.x < mx + mw
						&& ev.mouse_move.y > my && ev.mouse_move.y < my + mh) {
		...
		if (!last_color_time
				|| (int) (cur_time.diff_time(last_color_time) * 1000) > 120) {
			if (last_color_time)
			...
			screen->rectangle(mx + 1, by1, mx + mw - 2, by2,
					wm->bright_color());
		...
		if (ev.type == EV_MESSAGE) {
			...
			case ID_SFX_UP:
				...
				if (sound_avail & SFX_INITIALIZED)
					cache.sfx(cache.reg(s, s, SPEC_EXTERN_SFX, 1))->play(
							sfx_volume);
				break;
			case ID_SFX_DOWN:
				sfx_volume -= 16;
		...
		case ID_SHOW_SELL:
			if (!volume_window) {
				...
				if (title_screen >= 0) {
					...
					tit->put_image(screen,
							screen->Size().x / 2 - tit->Size().x / 2,
...
static ico_button *load_icon(int num, int id, int x, int y, int &h,
		ifield *next, char const *key) {
	char name[20];
				...
				load_icon(3, ID_EASY, x, y, h,
						load_icon(8, ID_MEDIUM, x, y, h,
								load_icon(9, ID_HARD, x, y, h,
										load_icon(10, ID_EXTREME, x, y, h, NULL,
												"ic_extreme"), "ic_hard"),
								"ic_medium"), "ic_easy"), NULL);
		y += h;
