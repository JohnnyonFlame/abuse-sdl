...
extern int text_draw(int y, int x1, int y1, int x2, int y2, char const *buf,
	...
	for (int y = 0; y < mask->Size().y; y++) {
		...
		for (x = 0; x < size; x++, rem++) {
			*(lrem++) = *(sl_start++);
			...
			if (x <= size / 2)
				*rem =
						(int) (sqrt(x / (double) (size * 2.0)) * map_width / 2.0);
			else
				*rem = map_width / 2
						- (int) (sqrt((size - x) / (double) (size * 2.0))
								* map_width / 2.0);
...
void scan_map(image *screen, int sx, int sy, image *im1, image *im2,
		int fade256, int32_t *paddr, mask_line *p, int mask_height, int xoff,
		int coff) {
	int x1 = 10000, x2 = 0;
	int iw = im1->Size().x;
	uint16_t r, off;
	...
	for (; y < mask_height; y++) {
		...
		for (; x < n->size; x++, sl++, rem++, l++) {
			...
			int r3 = r1 + (r2 - r1) * fade256 / 256, g3 = g1
					+ (g2 - g1) * fade256 / 256, b3 = b1
					+ (b2 - b1) * fade256 / 256;
...
void show_end2() {
	...
	int planet2 = cache.reg("art/fore/endgame.spe", "dead_planet", SPEC_IMAGE,
			1);
	int mask = cache.reg("art/fore/endgame.spe", "mask", SPEC_IMAGE, 1);
	int ship = cache.reg("art/fore/endgame.spe", "ship", SPEC_IMAGE, 1);

	int explo_snd = lnumber_value(
			LSymbol::FindOrCreate("P_EXPLODE_SND")->GetValue());
	int space_snd = lnumber_value(
			LSymbol::FindOrCreate("SPACE_SND")->GetValue());
	int zip_snd = lnumber_value(
			LSymbol::FindOrCreate("SHIP_ZIP_SND")->GetValue());

	mask_line *p = make_mask_lines(cache.img(mask),
			cache.img(planet)->Size().x);
	...
	screen->clear();
	int c[4] = { pal->find_closest(222, 222, 22), pal->find_closest(200, 200,
			200), pal->find_closest(100, 100, 100), pal->find_closest(64, 64,
			64) };
	...
	if (old_pal) {
		for (i = 0; i < 256; i++)
			paddr[i] = (old_pal->red(i) << 16) | (old_pal->green(i) << 8)
					| (old_pal->blue(i));
	} else {
		for (i = 0; i < 256; i++)
			paddr[i] = (pal->red(i) << 16) | (pal->green(i) << 8)
					| (pal->blue(i));
	}

	int dx = (xres + 1) / 2 - 320 / 2, dy = (yres + 1) / 2 - 200 / 2;

	scan_map(screen, ex, ey, cache.img(planet), cache.img(planet2), 0, paddr, p,
	...
	for (i = 0; i < 80;) {
		...
		if (new_time.diff_time(&old_time) > 0.1) {
			...
			if (i >= 30 && i <= 37) {
				...
				cache.fig(explo_frames1[i - 30])->forward->PutImage(tcopy,
						vec2i(100, 50));
				scan_map(screen, ex, ey, tcopy, cache.img(planet2), 0, paddr, p,
						cache.img(mask)->Size().y, eoff, coff);
			} else
				scan_map(screen, ex, ey, cache.img(planet), cache.img(planet2),
						0, paddr, p, cache.img(mask)->Size().y, eoff, coff);
			if (i > 38) {
				int t = i - 38;
				image *s = cache.img(ship);
				int nw = s->Size().x * (t + 2) / 16, nh = s->Size().y * (t + 2)
						/ 16;

				scale_put_trans(s, screen, ex - (i - 38) * 5,
						ey + cache.img(mask)->Size().y / 2 + t * 4, nw, nh);
				if (i == 77)
	...
	for (i = 0; i < 200;) {
		...
		if (new_time.diff_time(&old_time) > 0.1) {
			...
			scan_map(screen, ex, ey, cache.img(planet), cache.img(planet2),
					i * 256 / 200, paddr, p, cache.img(mask)->Size().y, eoff,
					coff);
			...
			if (i < 150 || (i < 170 && ((i - 149) % 2) == 0)
				...
				clist =
						new ex_char(
								ex
										+ jrand()
												% (cache.img(mask)->Size().x
														- cache.img(mask)->Size().x
																/ 3),
								ey
										+ jrand()
												% (cache.img(mask)->Size().y
														- cache.img(mask)->Size().y
																/ 3), 0, 1,
								clist);
				if (sound_avail & SFX_INITIALIZED)
...
//            ey+jrand()%(cache.img(mask)->Size().y,0,1,clist);
			...
			for (; c;) {
				...
				if (c->frame > 6) {
					...
					last = c;
					if (c->char_num)
						cache.fig(explo_frames2[c->frame])->forward->PutImage(
								screen, vec2i(c->x, c->y));
	...
	do {
		...
		if (new_time.diff_time(&old_time) > 0.1) {
			...
			old_time.get_time();
			scan_map(screen, ex, ey, cache.img(planet), cache.img(planet2), 256,
					paddr, p, cache.img(mask)->Size().y, eoff, coff);
			eoff += 2;
			if (eoff >= 320)
				eoff -= 320;
			coff += 1;
	...
	for (i = 0; i < 320 && ev.type != EV_KEY; i++) {
		...
		scan_map(screen, ex, ey, cache.img(planet), cache.img(planet2), 256,
				paddr, p, cache.img(mask)->Size().y, eoff, coff);
		text_draw(205 - i, dx + 10, dy, dx + 319 - 10, dy + 199,
				lstring_value(end_plot), wm->font(), cmap, wm->bright_color());
		wm->flush_screen();
		time_marker now;
...
void share_end() {
	fade_out(16);
	image blank(vec2i(2, 2));
	blank.clear();
	wm->set_mouse_shape(blank.copy(), 0, 0); // don't show mouse
	screen->clear();

	image *im = cache.img(
			cache.reg("art/fore/endgame.spe", "tbc", SPEC_IMAGE, 1));
	...
	int dx = (xres + 1) / 2 - im->Size().x / 2, dy = (yres + 1) / 2
			- im->Size().y / 2;
	im->put_image(screen, dx, dy);
	console_font->put_string(screen, xres / 2 + 35,
			yres / 2 + 100 - console_font->height() - 2, lstring_value(to_be));
	fade_in(NULL, 32);
	...
	for (i = 0; i < 320 && ev.type != EV_KEY; i++) {
		im->put_image(screen, dx, dy);
		console_font->put_string(screen, xres / 2 + 35,
				yres / 2 + 100 - console_font->height() - 2,
				lstring_value(to_be));

		text_draw(205 - i, dx + 10, dy, dx + 319 - 10, dy + 199,
				lstring_value(mid_plot), wm->font(), cmap, wm->bright_color());
		wm->flush_screen();
		time_marker now;
		while (now.diff_time(&start) < 0.18)
			now.get_time();
...
void show_end() {
	...
	image *im = cache.img(
			cache.reg("art/fore/endgame.spe", "end.pcx", SPEC_IMAGE, 1));
