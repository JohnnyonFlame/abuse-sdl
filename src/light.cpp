...
light_source *light_source::copy() {
	next = new light_source(type, x, y, inner_radius, outer_radius, xshift,
			yshift, next);
	return next;
}
...
light_source::light_source(char Type, int32_t X, int32_t Y,
		int32_t Inner_radius, int32_t Outer_radius, int32_t Xshift,
...
light_source *add_light_source(char type, int32_t x, int32_t y, int32_t inner,
		int32_t outer, int32_t xshift, int32_t yshift) {
	first_light_source = new light_source(type, x, y, inner, outer, xshift,
			yshift, first_light_source);
...
void calc_tint(uint8_t *tint, int rs, int gs, int bs, int ra, int ga, int ba,
		palette *pal) {
	palette npal;
	...
	if (recalc) {
		...
		for (; color < 256; color++) {
			...
			for (int intensity = 63; intensity >= 0; intensity--) {
				if (r > 0 || g > 0 || b > 0)
					white_light[intensity * 256 + color] = pal->find_closest(r,
							g, b);
				else
		...
		for (i = 0, c = tints[TTINTS - 1]; i < 256; i++, c++) // reverse green
				{
			int r = pal->red(i) / 2, g = 255 - pal->green(i) - 30, b =
					pal->blue(i) * 3 / 5 + 50;
			if (g < 0)
				g = 0;
			if (b > 255)
		...
		for (i = 0; i < 256; i++) {
			int r = pal->red(i) + (255 - pal->red(i)) / 2, g = pal->green(i)
					+ (255 - pal->green(i)) / 2, b = pal->blue(i)
					+ (255 - pal->blue(i)) / 2;
...
void add_light(light_patch *&first, int32_t x1, int32_t y1, int32_t x2,
		int32_t y2, light_source *who) {
	light_patch *next;
	light_patch *p = first;
	for (; p; p = next) {
		next = p->next;
		...
		if (x1 >= p->x1 && y1 >= p->y1 && x2 <= p->x2 && y2 <= p->y2) {
			...
			p->total++;
			p->lights = (light_source **) realloc(p->lights,
					sizeof(light_source *) * p->total);
		...
		if (!(x2 < p->x1 || y2 < p->y1 || x1 > p->x2 || y1 > p->y2)) {
			int ax1, ay1, ax2, ay2;
			if (x1 < p->x1) {
				add_light(first, x1, Max(y1, p->y1), p->x1 - 1, Min(y2, p->y2),
						who);
				ax1 = p->x1;
			} else
				ax1 = x1;
			...
			if (x2 > p->x2) {
				add_light(first, p->x2 + 1, Max(y1, p->y1), x2, Min(y2, p->y2),
						who);
				ax2 = p->x2;
			} else
				ax2 = x2;
...
light_patch *find_patch(int screenx, int screeny, light_patch *list) {
	for (; list; list = list->next) {
		if (screenx >= list->x1 && screenx <= list->x2 && screeny >= list->y1
				&& screeny <= list->y2)
			return list;
	}
	return NULL;
...
light_patch *make_patch_list(int width, int height, int32_t screenx,
		int32_t screeny) {
	...
	for (light_source *f = first_light_source; f; f = f->next) // determine which lights will have effect
			{
		int32_t x1 = f->x1 - screenx, y1 = f->y1 - screeny, x2 = f->x2
				- screenx, y2 = f->y2 - screeny;
		if (x1 < 0)
			x1 = 0;
		if (y1 < 0)
			y1 = 0;
...
inline void MAP_2PUT(uint8_t * in_addr, uint8_t * out_addr, uint8_t * remap,
		int w) {
	while (w--) {
...
void remap_line_asm2(uint8_t *addr, uint8_t *light_lookup, uint8_t *remap_line,
		int count)
...
inline void put_8line(uint8_t *in_line, uint8_t *out_line, uint8_t *remap,
		uint8_t *light_lookup, int count) {
	uint8_t v;
	int x;
	for (x = 0; x < count; x++) {
...
void light_screen(image *sc, int32_t screenx, int32_t screeny,
		uint8_t *light_lookup, uint16_t ambient) {
	...
	light_patch *first = make_patch_list(cx2 - cx1, cy2 - cy1, screenx,
			screeny);
...
//      f=f->next;
		...
		if (suffix) {
			light_patch *lp = f;
			for (;
					(lp->y1 > y - cy1 || lp->y2 < y - cy1 || lp->x1 > suffix_x
							|| lp->x2 < suffix_x); lp = lp->next)
				;
			uint8_t * caddr = (uint8_t *) screen_line + cx2 - cx1 - suffix;
			uint8_t *r = light_lookup
					+ (((int32_t) calc_light_value(lp, suffix_x + screenx,
							calcy) << 8));
			switch (todoy) {
			case 4: {
		...
		if (prefix) {
			light_patch *lp = f;
			for (;
					(lp->y1 > y - cy1 || lp->y2 < y - cy1 || lp->x1 > prefix_x
							|| lp->x2 < prefix_x); lp = lp->next)
				;

			uint8_t *r = light_lookup
					+ (((int32_t) calc_light_value(lp, prefix_x + screenx,
							calcy) << 8));
		...
		for (x = prefix, count = 0; count < remap_size;
				count++, x += 8, rem++) {
			light_patch *lp = f;
			for (;
					(lp->y1 > y - cy1 || lp->y2 < y - cy1 || lp->x1 > x
							|| lp->x2 < x); lp = lp->next)
				;
...
void double_light_screen(image *sc, int32_t screenx, int32_t screeny,
		uint8_t *light_lookup, uint16_t ambient, image *out, int32_t out_x,
		int32_t out_y) {
...
void read_lights(spec_directory *sd, bFILE *fp, char const *level_name) {
	...
	if (se) {
		...
		while (t) {
			...
			light_source *p = new light_source(ty, x, y, ir, ora, xshift,
					yshift, NULL);
