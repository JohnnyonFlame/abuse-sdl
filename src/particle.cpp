...
int defun_pseq(void *args) {
	...
	current_space = sp;
	pseqs = (part_sequence **) realloc(pseqs,
			sizeof(part_sequence *) * (total_pseqs + 1));
...
void draw_panims(view *v) {
	for (part_animation *p = first_anim; p; p = p->next) {
		cache.part(p->seq->frames[p->frame])->draw(screen,
				p->x - v->xoff() + v->cx1, p->y - v->yoff() + v->cy1, p->dir);
	}
}
...
void scatter_line(int x1, int y1, int x2, int y2, int c, int s) {
	...
	long xo = x1 << 16, yo = y1 << 16, dx = ((x2 - x1) << 16) / t, dy = ((y2
			- y1) << 16) / t, x, y;
