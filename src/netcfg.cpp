...
void net_configuration::cfg_error(char const *msg) {
	Jwindow *j = wm->new_window(-1, 0, -1, -1,
			new info_field(0, 0, 0, msg,
					new button(0, 30, CFG_ERR_OK, symbol_str("ok_button"),
	...
	} while (ev.type != EV_MESSAGE || ev.message.id != CFG_ERR_OK
			|| ev.type == EV_CLOSE_WINDOW
			|| (ev.type == EV_KEY && ev.key == JK_ALT_L));
	wm->close_window(j);
	wm->flush_screen();
}
...
int net_configuration::confirm_inputs(InputManager *i, int server) {
	if (server) {
		...
		if (sscanf(i->get(NET_KILLS)->read(), "%d", &kl) != 1 || kl < 1
				|| kl > 99) {
...
void net_configuration::error(char const *message) {
	...
	image *ns = cache.img(
			cache.reg("art/frame.spe", "net_screen", SPEC_IMAGE, 1));
	...
	int fx = x + ns_w / 2 - strlen(message) * fnt->width() / 2, fy = y
			+ ns_h / 2 - fnt->height();
...

	{
		...
		int bx = x + ns_w / 2 - strlen(ok) * fnt->width() / 2 - 3, by = y
				+ ns_h / 2 + fnt->height() * 3;
		...
		do {
			...
			if ((ev.type == EV_KEY && (ev.key == JK_ALT_L || ev.key == JK_CTRL_L))
					|| ev.type == EV_MESSAGE)
				done = 1;
		} while (!done);
	}
...
ifield *net_configuration::center_ifield(ifield *i, int x1, int x2,
		ifield *place_below) {
	int X1, Y1, X2, Y2;
	i->area(X1, Y1, X2, Y2);
	i->x = (x1 + x2) / 2 - (X2 - X1) / 2;
...
int net_configuration::get_options(int server) {
	...
	ns->put_image(screen, x, y);
	JCFont *fnt = wm->font();
	image *ok_image = cache.img(
			cache.reg("art/frame.spe", "dev_ok", SPEC_IMAGE, 1))->copy(),
			*cancel_image =
					cache.img(
	...
	if (server) {
		list = center_ifield(
				new text_field(x, y + 30, NET_NAME, symbol_str("your_name"),
						"************************", name, list), x, x + ns_w,
				NULL);
		list = center_ifield(
				new text_field(0, 0, NET_SERVER_NAME, symbol_str("server_name"),
						"************************", game_name, list), x,
				x + ns_w, list);
		list = center_ifield(
				new info_field(0, 0, 0, symbol_str("min_play"), list), x,
				x + ns_w, list);
		...
		list = center_ifield(
				new info_field(0, 0, 0, symbol_str("max_play"), list), x,
				x + ns_w, list);
		...
		list = center_ifield(
				new info_field(0, 0, 0, symbol_str("level_size"), list), x,
				x + ns_w, list);
		...
		list = center_ifield(
				new text_field(0, 0, NET_KILLS, symbol_str("kills_to_win"),
						"***", "25", list), x, x + ns_w, list);

	} else {
		list = center_ifield(
				new text_field(x, y + 80, NET_NAME, symbol_str("your_name"),
						"************************", name, list), x, x + ns_w,
				NULL);
	}

	list = new button(x + 80 - 17, y + ns_h - 20 - fnt->height(), NET_OK,
			ok_image, list);
	list = new button(x + 80 + 17, y + ns_h - 20 - fnt->height(), NET_CANCEL,
			cancel_image, list);
...

	{
		...
		do {
			...
			if (ev.type == EV_KEY && ev.key == JK_ALT_L)
				done = 1;
...
int net_configuration::input() // pulls up dialog box and input fileds
{
	...
	wm->font()->put_string(screen,
			x + ns_w / 2 - strlen(nw_s) * fnt->width() / 2,
			y + 21 / 2 - fnt->height() / 2, nw_s, wm->medium_color());
	{

		char const *server_str = symbol_str("server");
		button *sb = new button(x + 40, y + ns_h - 23 - fnt->height(),
				NET_SERVER, server_str, NULL);

		if (main_net_cfg
				&& (main_net_cfg->state == CLIENT
						|| main_net_cfg->state == SERVER))
			sb = new button(x + 40, y + ns_h - 9 - fnt->height(), NET_SINGLE,
					symbol_str("single_play"), sb);
		...
		do {
			if (wm->event_waiting()) {
				...
				if (ev.type == EV_MESSAGE) {
					...
					case NET_SINGLE:
						...
						break;
					default:
						if (ev.message.id >= NET_GAME
								&& ev.message.id < NET_GAME + MAX_GAMES) {
							join_game = ev.message.id - NET_GAME;
							done = 1;
						}
					}
				} else if (ev.type == EV_KEY && ev.key == JK_ALT_L) {
					done = 1;
				} else {
			...
			if (total_games < MAX_GAMES && now.diff_time(&start) > 0.5) {
				...
				if (find) {
					...
					inm.add(
							new button(x + ns_w / 2 - bw / 2, y + button_y,
									NET_GAME + total_games, name, NULL));
		...
		if (join_game >= 0) {
			if (get_options(0)) {
				...
				if (still_there) {
					game_addr[join_game]->store_string(server_name,
							sizeof(server_name));
