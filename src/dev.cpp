...
char backw_on = 0, forew_on = 0, show_menu_on = 0, ledit_on = 0, pmenu_on = 0,
		omenu_on = 0, commandw_on = 0, tbw_on = 0, searchw_on = 0,
		small_render_on = 0, interpolate_draw = 0, disable_autolight = 0,
		fps_on = 0, profile_on = 0, show_names = 0, fg_reversed = 0, raise_all;
...
int dev_del, dev_move, dev_char_left, dev_char_right, dev_back, dev_front,
		dev_ok, dev_copy, dev_brain, dev_lights, dev_objects, dev_ai,
		dev_mode_icon[DEV_MODES], dev_forward, dev_backward;
...
public:
	amb_cont(int X, int Y, ifield *Next) :
			scroller(X, Y, ID_NULL, 100, wm->font()->height() + 2, 0, 64, Next) {
		if (player_list)
...
int confirm_quit() {
	...
	ok_image =
			cache.img(cache.reg("art/frame.spe", "dev_ok", SPEC_IMAGE, 1))->copy();
	cancel_image = cache.img(
			cache.reg("art/frame.spe", "cancel", SPEC_IMAGE, 1))->copy();

	quitw = wm->new_window(xres / 2 + 40, yres / 2, 80, -1,
			new button(10, wm->font()->height() + 4, ID_QUIT_OK, ok_image,
					new button(38, wm->font()->height() + 4, ID_CANCEL,
							cancel_image,
							new info_field(2, 2, ID_NULL, symbol_str("sure?"),
									NULL))), symbol_str("quit_title"));
...
#define macro_to(case, no) ((no < 256) ? to##case(no) : no)
		...
		if (ev.type == EV_MESSAGE && ev.message.id == ID_QUIT_OK)
			...
			fin = 1;
				...
				&& macro_to(upper, ev.key) == macro_to(upper, *symbol_str("YES")))
			fin = quit = 1;
		else if (ev.type == EV_KEY
				&& macro_to(upper, ev.key) == macro_to(upper, *symbol_str("NO")))
			fin = 1;
...
static void double_render() {
	...
	small_render = new image(
			vec2i(the_game->first_view->cx2 - the_game->first_view->cx1 + 1,
					the_game->first_view->cy2 - the_game->first_view->cy1 + 1),
			NULL, 2);
}
...
void dev_controll::search_forward() {
	if (search_window) // if no window then we can't get the object name
	{
		...
		if (type == -1) {
			...
			int loop = 0;
			if (!find) {
				for (first = current_level->first_object(); first && !find;
						first = first->next) {
					if (first->otype == type)
						find = first;
				}
			...
			if (find) {
				...
				the_game->show_help(
						"No object matching name exsist in level\n");
...
int32_t dev_controll::snap_y(int32_t y) {
	if (wm->key_pressed(JK_CTRL_L) || wm->key_pressed(JK_CTRL_R))
		...
		return y - (y % the_game->ftile_height()) + the_game->ftile_height() / 2
				- 1;
	else
		return y;
}
...
void dev_controll::make_ambient() {
	...
	ambw = wm->new_window(prop->getd("ambient x", -1),
			prop->getd("ambient y", -1), -1, -1, new amb_cont(0, 0, NULL),
			"ambient");
}
...
static void load_dev_icons() {
	...
	for (int i = 0; i < DEV_MODES; i++)
		dev_mode_icon[i] = cache.reg(artf, dev_mode_icon_names[i], SPEC_IMAGE,
				0);

}

void scale_put(image *im, image *screen, int x, int y, short new_width,
		short new_height) {
	unsigned char *sl1, *sl2;
	int32_t xstep = (im->Size().x << 16) / new_width, ystep = (im->Size().y
			<< 16) / new_height, iy, ix, sx, ix_start, iy_start;
	screen->AddDirty(x, y, x + new_width, y + new_height);

	int cx1, cy1, cx2, cy2;
	screen->GetClip(cx1, cy1, cx2, cy2);
	if (cx1 > cx2 || cy1 > cy2 || x > cx2 - 1 || y > cy2 - 1
			|| x + new_width <= cx1 || y + new_height <= cy1)
		return;
	if (x < cx1) {
...
void scale_put_trans(image *im, image *screen, int x, int y, short new_width,
		short new_height) {
	unsigned char *sl1, *sl2;
	int32_t xstep = (im->Size().x << 16) / new_width, ystep = (im->Size().y
			<< 16) / new_height, iy, ix, sx, ix_start, iy_start;
	...
	if (dev & EDIT_MODE) {
		...
		if (dev & DRAW_LINKS) {
			for (light_source *f = first_light_source; f; f = f->next) {
				if (f->x - vx >= 0 && f->x - vx <= (v->cx2 - v->cx1 + 1)
						&& f->y - vy >= 0
						&& f->y - vy <= (v->cy2 - v->cy1 + 1)) {
					image *im = cache.img(light_buttons[f->type]);
					im->put_image(screen, f->x - vx + v->cx1 - im->Size().x / 2,
							f->y - vy + v->cy1 - im->Size().y / 2, 1);
					screen->rectangle(f->x1 - vx + v->cx1, f->y1 - vy + v->cy1,
		...
		if (link_object) {
			int32_t rx1, ry1;
			the_game->game_to_mouse(link_object->x, link_object->y, v, rx1,
					ry1);
		...
		if (selected_light) {
			...
			int32_t rx1, ry1;
			the_game->game_to_mouse(selected_light->x, selected_light->y, v,
					rx1, ry1);
			screen->rectangle(rx1 - l, ry1 - h, rx1 + l, ry1 + h,
					wm->bright_color());
		}
		...
		if (show_names)
			for (o = current_level->first_object(); o; o = o->next) {
				...
				console_font->put_string(screen,
						x1 - strlen(nm) * console_font->width() / 2, y1 + 2,
						nm);
			}
		...
		if (dev & DRAW_LINKS) {
			...
			for (o = current_level->first_object(); o; o = o->next) {
				...
				for (; i < o->total_objects(); i++) {
					...
					the_game->game_to_mouse(other->x, other->y, current_view,
							x2, y2);
		...
		if (selected_object) {
			...
			the_game->game_to_mouse(selected_object->x, selected_object->y,
					current_view, x1, y1);
			for (int i = 0; i < selected_object->total_objects(); i++) {
				game_object *other = selected_object->get_object(i);
				the_game->game_to_mouse(other->x, other->y, current_view, x2,
						y2);
				screen->line(x1, y1, x2, y2, light_connection_color);
			}
		}
...
void dev_controll::toggle_toolbar() {
	if (tbw) {
		...
		tool_picker *tp = new tool_picker(0, 0, ID_NULL, 5,
				(visual_object **) dev_mode_pict, dev_mode_ids, DEV_MODES, pal,
				pal, NULL);
...
void dev_controll::toggle_omenu() {
	...
	omenu = wm->new_window(prop->getd("objects x", 0),
			...
			new pick_list(0, 0, DEV_CREATE, yres / wm->font()->height() / 2,
					listable_objs, total_listable, 0, NULL,
...
void dev_controll::toggle_pmenu() {
	...
	pmenu = wm->new_window(prop->getd("pal x", 0), prop->getd("pal y", -1), -1,
			-1,
			new pick_list(0, 0, DEV_PALETTE, yres / wm->font()->height() / 2,
					pwin_list, total_pals, 0, NULL, cache.img(window_texture)));
}
	...
	search_window = wm->new_window(prop->getd("searchw x", -30),
			...
			new text_field(0, 0, ID_SEARCH_TEXT, "object name>",
					...
					new button(bw, wm->font()->height() + 5, ID_SEARCH_BACKWARD,
							...
							new button(bw * 3, wm->font()->height() + 5,
...
int open_owin = 0, open_fwin = 0, open_bwin = 0, start_edit = 0, start_nodelay =
		0, start_doubled = 0, start_mem = 0;
...
void dev_controll::do_command(char const *command, event &ev) {
		...
		if (current_level && current_level->first_active_object()) {
			...
			while (o) {
				dprintf("%s %d %d %d %d\n", object_names[o->otype], o->x, o->y,
						figures[o->otype]->rangex, figures[o->otype]->rangey);
				o = o->next_active;
			}
		}
	}
	...
	if (!strcmp(fword, "unchop")) {
		...
		if (rx >= 0 && ry >= 0) {
			if (sscanf(command, "%s%d%d", fword, &l, &h) == 3) {
				...
				h = (h + the_game->btile_height() - 1)
						/ the_game->btile_height();
	...
	if (!strcmp(fword, "delete")) {
		if (selected_object) {
			if (!(dev & EDIT_MODE)
					&& current_level->is_attacker(selected_object))
	...
	if (!strcmp(fword, "fg_select")) {
		...
		if (x >= 0 && y >= 0 && x < current_level->foreground_width()
			...
			if (forew)
				((tile_picker *) forew->read(DEV_FG_PICKER))->recenter(
						forew->screen);
			the_game->need_refresh();
		}
	}
...
void dev_controll::toggle_light_window() {
	...
	lightw =
			wm->new_window(prop->getd("light create x", 0),
					prop->getd("light create y", 0), -1, -1,
					new button_box(0, 0, DEV_LIGHT_BUTTON_BOX, 1,
							new button(bw * 0, bh * 0, DEV_LIGHT0,
									cache.img(light_buttons[0]),
									new button(bw * 1, bh * 0, DEV_LIGHT1,
											cache.img(light_buttons[1]),
											new button(bw * 2, bh * 0,
													DEV_LIGHT2,
													cache.img(light_buttons[2]),
													new button(bw * 0, bh * 1,
															DEV_LIGHT3,
															cache.img(
																	light_buttons[3]),
															new button(bw * 1,
							...
							new text_field(0, bh * 4, DEV_LIGHTW, "W ",
									...
									new text_field(0, bh * 4 + th * 1,
											DEV_LIGHTH, "H ", "******",
											prop->getd("light create h", 0),
											new text_field(0, bh * 4 + th * 2,
													DEV_LIGHTR1, "R1", "******",
													prop->getd(
															"light create r1",
															1),
...
void dev_controll::make_ai_window(game_object *o) {
	...
	if (figures[o->otype]->total_fields) {
		...
		for (; i < figures[o->otype]->total_fields; i++)
			if (strlen(figures[o->otype]->fields[i]->descript_name)
					> (unsigned) maxl)
		...
		for (i = 0; i < figures[o->otype]->total_fields; i++) {
			...
			for (int j = maxl
					- strlen(figures[o->otype]->fields[i]->descript_name); j;
					j--)
				strcat(tmp, " ");
			int er;
			ifield *p = new text_field(wl, wh, ID_NULL, tmp, "######",
					(double) o->get_var_by_name(
							figures[o->otype]->fields[i]->real_name, er), NULL);
			if (last)
				last->next = p;
			else
				first = p;
		...
		aiw = wm->new_window(prop->getd("ai x", 0), prop->getd("ai y", 0), -1,
				-1,
				new button(wl, owh - 20, DEV_AI_OK, cache.img(dev_ok), first),
				"ai");

	} else {
		aiw =
				wm->new_window(prop->getd("ai x", 0), prop->getd("ai y", 0), -1,
						-1,
						new button(wl, wh - 20, DEV_AI_OK, cache.img(dev_ok),
								new text_field(wl, wh + th * 0, DEV_AI_XVEL,
										symbol_str("ai_xvel"), "#####",
										(double) o->xvel(),
										new text_field(wl, wh + th * 1,
												DEV_AI_YVEL,
												symbol_str("ai_yvel"), "#####",
												(double) o->yvel(),
												new text_field(wl, wh + th * 2,
														DEV_AI_XACEL,
														symbol_str("ai_xacel"),
														"#####",
														(double) o->xacel(),
...
void dev_controll::area_handle_input(event &ev) {
	...
	if (ev.type == EV_MOUSE_BUTTON && ev.mouse_button) {
		...
		current_area = current_level->area_list = new area_controller(gx, gy,
				the_game->ftile_width(), the_game->ftile_height(),
...
void dev_controll::close_area_win(int read_values) {
	if (area_win) {
		...
		if (current_area && read_values) {
			...
			current_area->ambient_speed = atoi(
					area_win->read(DEV_AREA_AMBIENT_SPEED));
			current_area->view_xoff = atoi(area_win->read(DEV_AREA_VIEW_XOFF));
			current_area->view_yoff = atoi(area_win->read(DEV_AREA_VIEW_YOFF));
			current_area->view_xoff_speed = atoi(
					area_win->read(DEV_AREA_VIEW_XOFF_SPEED));
			current_area->view_yoff_speed = atoi(
					area_win->read(DEV_AREA_VIEW_YOFF_SPEED));
		}
...
void dev_controll::pick_handle_input(event &ev) {
	...
	if (ev.type == EV_MOUSE_BUTTON && ev.mouse_button) {
		...
		if (find && current_area && dc) {
			if (area_win)
				close_area_win(0);
			int wl = 0, wh = 0, th = wm->font()->height() + 12, bw = cache.img(
					dev_ok)->Size().x + 10;
			area_win =
					wm->new_window(prop->getd("area_box x", 0),
							prop->getd("area_box y", 0), -1, -1,

							new button(wl + bw * 0, wh - 8, DEV_AREA_OK,
									cache.img(dev_ok),
									new button(wl + bw * 1, wh - 8,
											DEV_AREA_DELETE, cache.img(dev_del),

											new text_field(wl, wh + th * 1,
													DEV_AREA_AMBIENT,
													symbol_str("a_ambient"),
													"******",
													current_area->ambient,
													new text_field(wl,
															wh + th * 2,
...
void dev_controll::handle_event(event &ev) {
	...
	case DEV_MOVE_OBJECT: {
		if (!edit_object) {
			...
			if (ev.type == EV_MOUSE_MOVE) {
				the_game->mouse_to_game(last_demo_mx, last_demo_my,
						edit_object->x, edit_object->y);
			...
			if (ev.window == NULL && ev.type == EV_KEY && ev.key == 'd') {
				int32_t xv = 0, yv = 100;
				edit_object->try_move(edit_object->x, edit_object->y, xv, yv,
						1);
				edit_object->y += yv;
				state = DEV_SELECT;
				selected_object = edit_object = NULL;
		...
		if (edit_light) {
			if (ev.type == EV_MOUSE_MOVE) {
				...
				case '-': {
					...
					} else if (edit_light->outer_radius
	...
	case DEV_DRAG_AREA_BOTTOM: {
		if (current_area) {
			...
			if (gx > current_area->x && gy > current_area->y) {
				if (gx - current_area->x != current_area->w
						|| gy - current_area->y != current_area->h) {
					the_game->need_refresh();
		...
		if (dev & EDIT_MODE) {
			...
			if (ev.window == NULL) {
				...
				if (edit_mode == ID_DMODE_DRAW) {
					if (ev.mouse_button == 1 && !selected_object
							&& !selected_light) {
						int32_t xs, ys;
						the_game->ftile_on(last_demo_mx, last_demo_my, xs, ys);
						if (xs >= 0 && ys >= 0
								&& xs < current_level->foreground_width()
								&& ys < current_level->foreground_height())
							current_level->put_fg(xs, ys,
									raise_all ?
											make_above_tile(cur_fg) : cur_fg);
						the_game->need_refresh();
					} else if (ev.mouse_button == 1 && !selected_object
							&& !selected_light) {
						int32_t xs, ys;
						the_game->btile_on(last_demo_mx, last_demo_my, xs, ys);
						if (xs >= 0 && ys >= 0
								&& xs < current_level->background_width()
			...
			if (ev.mouse_button) {
				if (selected_object) {
					...
					oedit =
							wm->new_window(prop->getd("oedit x", 0),
									prop->getd("oedit y", 0), -1, -1,
					...
					ledit =
							wm->new_window(prop->getd("ledit x", 0),
									prop->getd("ledit y", 0), -1, -1,
									new button_box(0, 0, ID_NULL, 1,
											new button(bw * 0, 0, DEV_LEDIT_OK,
													cache.img(dev_ok),
													new button(bw * 1, 0,
															DEV_LEDIT_MOVE,
															cache.img(dev_move),
															new button(bw * 2,
																	...
																	new button(
																			...
																			cache.img(
																					dev_del),
																			NULL)))),
											new text_field(0, bh, DEV_LEDIT_W,
													"W ", "******",
													edit_light->xshift,
													new text_field(0,
															bh + th * 1,
					...
					if (dlastx >= 0 && dlasty >= 0
							&& edit_mode == ID_DMODE_DRAW) {
						if ((dev & DRAW_FG_LAYER) && ev.mouse_button == 1) {
							the_game->ftile_on(last_demo_mx, last_demo_my, x,
									y);
							if (x >= 0 && y >= 0
									&& x < current_level->foreground_width()
									&& y < current_level->foreground_height())
								the_game->put_fg(x, y,
										raise_all ?
												make_above_tile(cur_fg) :
												cur_fg);
						}
						if ((dev & DRAW_BG_LAYER) && ev.mouse_button == 2) {
							the_game->btile_on(last_demo_mx, last_demo_my, x,
									y);
							if (x >= 0 && y >= 0
									&& x < current_level->background_width()
	...
	case EV_MESSAGE: {
		...
		case ID_LEVEL_LOAD: {
			if (!mess_win) {
				mess_win = file_dialog(symbol_str("level_name"),
						...
						symbol_str("cancel_button"), symbol_str("FILENAME"),
						ID_MESS_STR1);
		...
		case ID_LEVEL_SAVE: {
			if (current_level) {
				if (current_level->save(current_level->name(), 0)) {
					char msg[100];
					sprintf(msg, symbol_str("saved_level"),
							current_level->name());
					the_game->show_help(msg);
					the_game->need_refresh();
				}
		...
		case ID_LEVEL_SAVEAS: {
			if (!mess_win) {
				mess_win = file_dialog(symbol_str("saveas_name"),
						current_level ? current_level->name() : "untitled.spe",
						ID_LEVEL_SAVEAS_OK, symbol_str("ok_button"), ID_CANCEL,
						symbol_str("cancel_button"), symbol_str("FILENAME"),
						ID_MESS_STR1);
				wm->grab_focus(mess_win);
			}
		...
		case ID_LEVEL_RESIZE: {
			if (!mess_win) {
				...
				mess_win =
						wm->new_window(xres / 2, yres / 2, -1, -1,
								new text_field(0, h * 0, ID_MESS_STR1,
										symbol_str("width_"), "****",
										current_level ?
												current_level->foreground_width() :
												100,
										new text_field(0, h * 1, ID_MESS_STR2,
												symbol_str("height_"), "****",
												current_level ?
			...
			if (!mess_win) {
				...
				mess_win = wm->new_window(xres / 2, yres / 2, -1, -1,
						new text_field(0, h * 0, ID_RECORD_DEMO_FILENAME,
								"demo filename", "*******************",
								"demo.dat",
								new button(10, h * 2, ID_RECORD_DEMO_OK,
										symbol_str("ok_button"),
										new button(40, h * 2, ID_CANCEL,
		...
		case ID_RECORD_DEMO_OK: {
			demo_man.set_state(demo_manager::RECORDING,
					mess_win->read(ID_RECORD_DEMO_FILENAME));
			wm->push_event(new event(ID_CANCEL, NULL)); // close window
		}
			break;
			...
			if (!mess_win) {
				...
				mess_win = wm->new_window(xres / 2, yres / 2, -1, -1,
						new text_field(0, h * 0, ID_PLAY_DEMO_FILENAME,
								"demo filename", "*******************",
								"demo.dat",
								new button(10, h * 2, ID_PLAY_DEMO_OK,
										symbol_str("ok_button"),
										new button(40, h * 2, ID_CANCEL,
												symbol_str("cancel_button"),
												NULL))));
		...
		case ID_PLAY_DEMO_OK: {
			demo_man.set_state(demo_manager::PLAYING,
					mess_win->read(ID_PLAY_DEMO_FILENAME));
			wm->close_window(mess_win);
			mess_win = NULL;
		}
			break;
		...
		case ID_SET_SCROLL: {
			if (!mess_win) {
				...
				mess_win =
						wm->new_window(xres / 2, yres / 2, -1, -1,
								new text_field(0, h * 0, ID_MESS_STR1,
										symbol_str("x_mul"), "****", bg_xmul,
										new text_field(0, h * 1, ID_MESS_STR2,
												symbol_str("x_div"), "****",
												bg_xdiv,
												new text_field(0, h * 2,
														ID_MESS_STR3,
														symbol_str("y_mul"),
														"****", bg_ymul,
														new text_field(0, h * 3,
																ID_MESS_STR4,
																symbol_str(
																		"y_div"),
																"****", bg_ydiv,
		...
		case ID_SET_SCROLL_CHECK: {
			...
			if ((((float) tbg_xmul / (float) tbg_xdiv)
					< ((float) bg_xmul / (float) bg_xdiv))
					|| (((float) tbg_ymul / (float) tbg_ydiv)
							< ((float) bg_ymul / (float) bg_ydiv))) {
				int h = wm->font()->height() + 8;

				warn_win = wm->new_window(xres / 2 - 40, yres / 2 - 40, -1, -1,
						new info_field(0, 0, ID_NULL, symbol_str("back_loss"),
								new button(10, h * 4, ID_SET_SCROLL_OK,
										symbol_str("ok_button"),
										new button(40, h * 4, ID_WARN_CANCEL,
												symbol_str("cancel_button"),
												NULL))), symbol_str("WARNING"));
				wm->grab_focus(warn_win);
			} else
				wm->push_event(new event(ID_SET_SCROLL_OK, NULL));
		}
			break;
		case ID_WARN_CANCEL: {
		...
		case ID_ADD_PALETTE: {
			if (!mess_win) {
				...
				mess_win =
						wm->new_window(xres / 2, yres / 2, -1, -1,
								new text_field(0, h * 0, ID_MESS_STR1,
										symbol_str("ap_width"), "****", 2,
										new text_field(0, h * 1, ID_MESS_STR2,
												symbol_str("ap_height"), "****",
												2,
												new text_field(0, h * 2,
														ID_MESS_STR3,
														symbol_str("ap_name"),
		...
		case ID_ADD_PALETTE_OK: {
			char name[70];
			sprintf(name, "(add_palette \"%s\" %d %d)",
					mess_win->read(ID_MESS_STR3),
		...
		case DEV_CREATE: {
			int val = get_omenu_item(
					((pick_list *) ev.message.data)->get_selection());
			char cmd[100];
	...
	case EV_KEYRELEASE: {
		if (ev.key == JK_CTRL_L) {
			if (!edit_object && link_object && selected_object
...
//      case '/' : if (dev_console) dev_console->toggle(); break;
			...
			case 'A': {
				if (selected_object) {
					if (oedit)
						wm->push_event(new event(DEV_OEDIT_OK, NULL));
			...
			case 'C':
				if (selected_object && selected_object->controller() == NULL) {
					...
					wm->push_event(new event(DEV_OEDIT_COPY, NULL));
				}
				break;
			...
			case 'w': {
				...
				the_game->mouse_to_game(dlastx, dlasty, rx, ry);
				char msg[100];
				sprintf(msg, symbol_str("mouse_at"), rx, ry);
				the_game->show_help(msg);
				the_game->need_refresh();
			}
				break;
			case 'k': {
				if (selected_object && selected_object->total_objects())
					selected_object->remove_object(
							selected_object->get_object(0));
				the_game->need_refresh();
			}
				break;
			case 'K': {
				if (selected_object && selected_object->total_objects())
					selected_object->remove_object(
							selected_object->get_object(
									selected_object->total_objects() - 1));
				the_game->need_refresh();
			}
				break;
			case 'j': {
...
void pal_win::draw() {
	...
	if (me) {
		me->clear();
		image *im = new image(
				vec2i(the_game->ftile_width(), the_game->ftile_height()));
		int th = the_game->ftile_height() / scale, tw = the_game->ftile_width()
				/ scale;
	...
	if (ev.window && ev.window == me) {
		...
		case EV_MOUSE_BUTTON: {
			if (ev.mouse_button == 1) {
				int selx = (last_demo_mx - me->x - me->x1())
						/ (the_game->ftile_width() / scale), sely =
						(last_demo_my - me->y - me->y1())
								/ (the_game->ftile_height() / scale);
				if (selx >= 0 && sely >= 0 && selx < w && sely < h) {
					cur_fg = pat[selx + sely * w];
					if (dev_cont->forew)
						((tile_picker *) dev_cont->forew->read(DEV_FG_PICKER))->recenter(
								dev_cont->forew->screen);
				}
			...
			case ' ': {
				...
				for (xx = xs; xx < xs + w; xx++) {
					for (yy = ys; yy < ys + h; yy++) {
						if (xx >= 0 && yy >= 0
								&& xx < current_level->foreground_width()
								&& yy < current_level->foreground_height())
							the_game->put_fg(xx, yy,
									raise_all ?
											make_above_tile(pat[xx-xs+(yy-ys)*w]) :
											pat[xx - xs + (yy - ys) * w]);
					}
				}
			}
...
void pal_win::resize(int xa, int ya) {
	...
	npat = (unsigned short *) malloc(
			sizeof(unsigned short) * (w + xa) * (h + ya));
...
void pal_win::save(FILE *fp) {
	...
	fprintf(fp, "(add_palette \"%s\" %ld %ld %ld %ld %ld ", name, (long) w,
			(long) h, (long) x, (long) y, (long) scale);
	int i;
	for (i = 0; i < w * h; i++)
	...
	do {
		...
		if (fgvalue(sl[x]) == fcolor) {
			while (x > 0 && fgvalue(sl[x]) == fcolor)
				x--;
			if (fgvalue(sl[x]) != fgvalue(fcolor)
					&& x < current_level->foreground_width() - 1)
				x++;
			if (y > 0) {
			...
			do {
				...
				if (y > 0) {
					above = current_level->get_fgline(y - 1);
					if (x
							> 0&& fgvalue(above[x-1])!=fgvalue(fcolor) && fgvalue(above[x])==fgvalue(fcolor)) {
						r = new fill_rec(x, y - 1, recs);
						recs = r;
					}
				}
				if (y < current_level->foreground_height() - 1) {
					...
					if (x
							> 0&& fgvalue(below[x-1])!=fgvalue(fcolor) && fgvalue(below[x])==fgvalue(fcolor)) {
						r = new fill_rec(x, y + 1, recs);
						recs = r;
					}
				}
				x++;
			} while (fgvalue(sl[x]) == fgvalue(fcolor)
					&& x < current_level->foreground_width());
			x--;
			if (y > 0) {
				above = current_level->get_fgline(y - 1);
...
static int get_char_mem(int type, int print) {
	...
	for (int j = 0; j < MAX_STATE; j++) {
		if (figures[type]->has_sequence((character_state) j)) {
			int s =
					figures[type]->get_sequence((character_state) j)->MemUsage();
			if (print)
...
static pmi filemenu[] = { { "menu1_load", ID_LEVEL_LOAD, NULL, -1 }, { NULL, 0,
		NULL, -1 }, { "menu1_save", ID_LEVEL_SAVE, NULL, -1 }, { "menu1_saveas",
		ID_LEVEL_SAVEAS, NULL, -1 },
		{ "menu1_savegame", ID_GAME_SAVE, NULL, -1 }, { "menu1_new",
				ID_LEVEL_NEW, NULL, -1 }, { "menu1_resize", ID_LEVEL_RESIZE,
				NULL, -1 }, { NULL, 0, NULL, -1 }, { "menu1_suspend",
				ID_SUSPEND, NULL, -1 },
		{ "menu1_toggle", ID_PLAY_MODE, NULL, -1 }, { NULL, 0, NULL, -1 }, {
				"menu1_savepal", ID_EDIT_SAVE, NULL, -1 },
//      { "menu1_startc",ID_CACHE_PROFILE,NULL,-1},
//      { "menu1_endc",ID_CACHE_PROFILE_END,NULL,-1},
		{ NULL, 0, NULL, -1 }, { "menu1_quit", ID_QUIT, NULL, -1 }, { NULL, -1,
				NULL, -1 } };

static pmi editmenu[] = { { "menu2_light", ID_TOGGLE_LIGHT, NULL, -1 }, {
		"menu2_scroll", ID_SET_SCROLL, NULL, -1 }, { "menu2_center",
		ID_CENTER_PLAYER, NULL, -1 },
		{ "menu2_addpal", ID_ADD_PALETTE, NULL, -1 }, { "menu2_delay",
				ID_TOGGLE_DELAY, NULL, -1 },

		{ "menu2_god", ID_GOD_MODE, NULL, -1 }, { "menu2_clear",
				ID_CLEAR_WEAPONS, NULL, -1 }, { "menu2_mscroll",
				ID_MOUSE_SCROLL, &mouse_scrolling, -1 }, { "menu2_lock",
				ID_LOCK_PALETTES, &palettes_locked, -1 }, { "menu2_raise",
				ID_RAISE_ALL, &raise_all, -1 }, { "menu2_names",
				ID_TOGGLE_NAMES, &show_names, -1 },

		{ NULL, 0, NULL, -1 }, { "menu2_map", ID_TOGGLE_MAP, NULL, -1 },
...
static pmi winmenu[] = { { "menu3_fore", ID_WIN_FORE, &forew_on, -1 }, {
		"menu3_back", ID_WIN_BACK, &backw_on, -1 }, { "menu3_layers",
		ID_WIN_LAYERS, &show_menu_on, -1 }, { "menu3_light", ID_WIN_LIGHTING,
		&ledit_on, -1 }, { "menu3_pal", ID_WIN_PALETTES, &pmenu_on, -1 }, {
...
//      { "Search      (s)",    ID_SEARCH,      &searchw_on,-1},
		{ "menu3_prof", ID_PROFILE, &profile_on, -1 }, { "menu3_save",
				ID_SAVE_WINDOWS, NULL, -1 }, { NULL, -1, NULL, -1 } };
...
static pmenu_item *i_recurse(pmi *first) {
	if (first->id == -1)
		...
		return new pmenu_item(first->id,
				first->name ? symbol_str(first->name) : 0, first->on_off,
				first->key, i_recurse(first + 1));
}

static pmenu *make_menu(int x, int y) {
	return new pmenu(x, y,
			new pmenu_item(symbol_str("file_top"),
					new psub_menu(i_recurse(filemenu), NULL),
					new pmenu_item(symbol_str("edit_top"),
							new psub_menu(i_recurse(editmenu), NULL),
							new pmenu_item(symbol_str("window_top"),
									new psub_menu(i_recurse(winmenu), NULL),
									NULL))), screen);
}
