...
file_manager::file_manager(int argc, char **argv, net_protocol *proto) :
	...
	for (i = 1; i < argc; i++)
		if (!strcmp(argv[i], "-bastard")) // this bypasses filename security features
				{
			fprintf(stderr,
					"Warning : Security measures bypassed (-bastard)\n");
			no_security = 1;
		}
}
...
int file_manager::nfs_client::send_read() // return 0 if failure on socket, not failure to read
{
	if (file_fd >= 0 && sock) {
		...
		if (sock->ready_to_write()) {
			...
			do {
				read_total =
						size_to_read > (READ_PACKET_SIZE - 2) ?
								(READ_PACKET_SIZE - 2) : size_to_read;
...
file_manager::nfs_client::nfs_client(net_socket *sock, int file_fd,
		nfs_client *next) :
...
file_manager::remote_file::remote_file(net_socket *sock, char const *filename,
		char const *mode, remote_file *Next) :
		sock(sock) {
	next = Next;
	open_local = 0;
	...
	if (sock->read(&remote_file_fd, sizeof(remote_file_fd))
			!= sizeof(remote_file_fd)) {
