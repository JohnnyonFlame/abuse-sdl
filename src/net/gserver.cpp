...
void game_server::game_start_wait() {
	...
	while (!abort && total_players() < main_net_cfg->min_players) {
		if (last_count != total_players()) {
			...
			char msg[100];
			sprintf(msg, symbol_str("min_wait"),
					main_net_cfg->min_players - total_players());
			stat = wm->new_window(100, 50, -1, -1,
					new info_field(0, 0, ID_NULL, msg,
							new button(0, wm->font()->height() * 2, ID_CANCEL,
...
void game_server::check_collection_complete() {
	...
	if (got_all) // see if we have input from everyone, if so send it out
	{
		...
		for (c = player_list; c; c = c->next) // setup for next time, wait for all the input
			...
			if (c->has_joined()) {
				c->set_wait_input(1);
				game_sock->write(base->packet.data,
						base->packet.packet_size()
								+ base->packet.packet_prefix_size(),
						c->data_address);
...
int game_server::process_client_command(player_client *c) {
	...
	case CLCMD_REQUEST_RESEND: {
		...
		fprintf(stderr,
				"request for resend tick %d (game cur=%d, pack=%d, last=%d)\n",
...
int game_server::process_net() {
	...
	if ((base->input_state == INPUT_COLLECTING
		...
		if (from && bytes_received) {
			...
			if (bytes_received
					== use->packet_size() + use->packet_prefix_size()) {
...
//          { time_marker now,start; while (now.diff_time(&start)<5.0) now.get_time(); }
							...
							if (base->input_state != INPUT_RELOAD)
								add_client_input((char *) use->packet_data(),
										use->packet_size(), found);

						} else if (use->tick_received()
								== base->last_packet.tick_received()) {
							if (prot->debug_level(
									net_protocol::DB_IMPORTANT_EVENT))
							...
							game_sock->write(pack->data,
									pack->packet_size()
											+ pack->packet_prefix_size(),
									found->data_address);

						} else if (prot->debug_level(
								net_protocol::DB_MAJOR_EVENT))
							fprintf(stderr,
									"received stale packet (got %d, expected %d)\n",
	...
	for (c = player_list; c; c = c->next)
		if (c->comm->error()
				|| (c->comm->ready_to_read() && !process_client_command(c))) {
			c->set_delete_me(1);
			check_collection_complete();
		} else
			ret = 1;
...
int game_server::add_client(int type, net_socket *sock, net_address *from) {
	if (type == CLIENT_ABUSE) {
		...
		if (sock->read(&len, 1) != 1 || sock->read(name, len) != len
				|| sock->read(&cport, 2) != 2 || sock->write(&our_port, 2) != 2
