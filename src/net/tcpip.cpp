...
{
	int zz;
	if (setsockopt(fd, SOL_SOCKET, SO_BROADCAST, (char *) &zz, sizeof(zz))
			< 0) {
...
net_socket *tcpip_protocol::connect_to_server(net_address *addr,
...
//{{{
	...
	if (connect(socket_fd, (struct sockaddr *) &((ip_address *) addr)->addr,
...
net_socket *tcpip_protocol::create_listen_socket(int port,
		net_socket::socket_type sock_type)
//{{{
		{
	int socket_fd = socket(AF_INET,
...
//#endif
	...
	if (responder) {
		...
		for (i = 0; i < 5; i++) {
			...
			for (p_request p = servers.begin(); !found && p != servers.end();
					++p)
				if (*((*p)->addr) == *bcast)
					found = 1;
			for (p_request q = returned.begin(); !found && q != returned.end();
					++q)
				if (*((*q)->addr) == *bcast)
					found = 1;
	...
	if (responder->ready_to_read()) {
		...
		if (addr && len > 0) {
			...
			if (strcmp(buf, notify_response) == 0) {
				int found = 0;
				for (p_request p = servers.begin();
						!found && p != servers.end(); ++p)
					if (*((*p)->addr) == *addr)
						found = 1;
				for (p_request q = returned.begin();
						!found && q != returned.end(); ++q)
					if (*((*q)->addr) == *addr)
						found = 1;
...
//{{{
	...
	if (block) {
		...
		while (ret == 0) {
			...
			ret = ::select(FD_SETSIZE, &read_set, &write_set, &exception_set,
					NULL);
