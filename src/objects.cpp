...
obj_desc object_descriptions[TOTAL_OBJECT_VARS] =
		{ { "fade_dir", RC_8 }, { "frame_dir", RC_8 }, { "direction", RC_8 }, {
				"gravity_on", RC_8 }, { "fade_count", RC_8 },

		{ "fade_max", RC_8 }, { "active", RC_8 }, { "flags", RC_8 }, { "aitype",
				RC_8 }, { "xvel", RC_32 },

		{ "fxvel", RC_8 }, { "yvel", RC_32 }, { "fyvel", RC_8 }, { "xacel",
				RC_32 }, { "fxacel", RC_8 },

		{ "yacel", RC_32 }, { "fyacel", RC_8 }, { "x", RC_32 }, { "fx", RC_8 },
				{ "y", RC_32 },

				{ "fy", RC_8 }, { "hp", RC_16 }, { "mp", RC_16 },
				{ "fmp", RC_16 }, { "cur_frame", RC_16 },

				{ "aistate", RC_16 }, { "aistate_time", RC_16 }, { "targetable",
						RC_8 } };
...
int32_t game_object::get_var_by_name(char *name, int &error) {
	...
	for (i = 0; i < figures[otype]->tiv; i++) {
		if (!strcmp(
				lstring_value(((LSymbol *) figures[otype]->vars[i])->GetName()),
				name)) {
...
void game_object::morph_into(int type, void (*stat_fun)(int), int anneal,
		int frames) {
...
int game_object::decide() {
	if (figures[otype]->get_fun(OFUN_AI)) {
		...
		LObject *ret =
				((LSymbol *) figures[otype]->get_fun(OFUN_AI))->EvalFunction(
						NULL);
		if (profiling()) {
			time_marker now;
...
void game_object::do_damage(int amount, game_object *from, int32_t hitx,
		int32_t hity, int32_t push_xvel, int32_t push_yvel) {
	// No friendly fire
...
void game_object::damage_fun(int amount, game_object *from, int32_t hitx,
		int32_t hity, int32_t push_xvel, int32_t push_yvel) {
	if (!hurtable() || !alive())
		return;
...
int game_object::facing_attacker(int attackerx) {
	return ((attackerx < x && direction < 0)
			|| (attackerx >= x && direction > 0));

}

void game_object::picture_space(int32_t &x1, int32_t &y1, int32_t &x2,
		int32_t &y2) {
...
void game_object::map_draw() {
	if (figures[otype]->get_fun(OFUN_MAP_DRAW)) {
		...
		((LSymbol *) figures[otype]->get_fun(OFUN_MAP_DRAW))->EvalFunction(
				NULL);
		if (profiling()) {
			time_marker now;
			profile_add_time(this->otype, now.diff_time(prof1));
...
void game_object::draw_trans(int count, int max) {
	...
	cpict->PutFade(screen,
			vec2i(
					(direction < 0 ?
							x - (cpict->Size().x - x_center() - 1) :
							x - x_center()) - current_vxadd,
					y - cpict->Size().y + 1 - current_vyadd), count, max,
		...
		cpict->PutFadeTint(screen,
				vec2i(
						(direction < 0 ?
								x - (cpict->Size().x - x_center() - 1) :
								x - x_center()) - current_vxadd,
						y - cpict->Size().y + 1 - current_vyadd), fade_count(),
				fade_max(), cache.ctint(tint_id)->data, color_table,
				the_game->current_palette());
...
game_object *game_object::try_move(int32_t x, int32_t y, int32_t &xv,
		int32_t &yv, int checks) {
	if (xv || yv) // make sure they are suggesting movement
		...
		if (checks & 2) {
			...
			if (!stoppable())
				who2 = current_level->all_boundary_setback(this, x, y - h + 1,
						x2, y2);
			else
				who2 = current_level->boundary_setback(this, x, y - h + 1, x2,
						y2);
			xv = x2 - x;
			yv = y2 - y + h - 1;
		}
...
void *game_object::float_tick() // returns 1 if you hit something, 0 otherwise
{
	...
	int32_t xv = xvel() + xacel() + (fxv >> 8), yv = yvel() + yacel()
			+ (fyv >> 8);
	...
	if (fxv || fyv || xv || yv) // don't even try if there is no velocity
		...
		if (old_nxv != nxv || old_nyv != nyv) {
			...
			if (old_nxv == 0) {
				...
				if (!ret)
					ret |= (old_nyv > 0 ? BLOCKED_DOWN : BLOCKED_UP)
							| (old_nxv > 0 ? BLOCKED_RIGHT : BLOCKED_LEFT);
...
int game_object::tick() // returns blocked status
{
	...
	if (old_vx || old_vy) {
		...
		if (xv != old_vx || yv != old_vy) // he collided with something
			...
			if (xvel() != old_vx && state != run_jump_fall
					&& state != end_run_jump) {
				set_xacel(0);
				set_fxacel(0);
			}
...
game_object *create(int type, int32_t x, int32_t y, int skip_constructor,
		int aitype) {
	...
	if (figures[type]->get_fun(OFUN_CONSTRUCTOR) && !skip_constructor) {
		...
		((LSymbol *) figures[type]->get_fun(OFUN_CONSTRUCTOR))->EvalFunction(
				NULL);
		if (profiling()) {
			time_marker now;
...
int game_object::move(int cx, int cy, int button) {
	...
	if (figures[otype]->get_fun(OFUN_MOVER)) // is a lisp move function defined?
		...
		LObject *r =
				((LSymbol *) figures[otype]->get_fun(OFUN_MOVER))->EvalFunction(
						lcx);
		if (profiling()) {
			time_marker now;
			profile_add_time(this->otype, now.diff_time(prof1));
		...
		if (item_type(r) != L_NUMBER) {
			((LObject *) r)->Print();
			lbreak(
					"Object %s did not return a number from its mover function!\n"
...
game_object *game_object::bmove(int &whit, game_object *exclude) {
	...
	int32_t nx = x + xvel(), nfx = fx() + fxvel(), ny = y + yvel(), nfy = fy()
			+ fyvel();
	nx += nfx >> 8;
	ny += nfy >> 8;
	...
	game_object *ret = current_level->all_boundary_setback(exclude, x, y, nx,
			ny);
	x = nx;
	y = ny;
	set_fx(nfx & 0xff);
...
void game_object::change_type(int new_type) {
	...
	if (figures[new_type]->get_fun(OFUN_CONSTRUCTOR)) {
		...
		((LSymbol *) figures[new_type]->get_fun(OFUN_CONSTRUCTOR))->EvalFunction(
				NULL);
		if (profiling()) {
			time_marker now;
