...
int nforetiles, nbacktiles, f_wid, f_hi, b_wid, b_hi, total_songs = 0,
		sfx_volume, music_volume, sound_avail = 0;
...
int border_tile, window_texture, raise_volume, lower_volume, record_button,
		play_button, music_button, sfx_button, window_colors, pause_image,
		damage_pict, block_pict, vmm_image, earth, earth_mask, clouds,
	...
	if (!fp->open_failure()) {
		...
		if (bt)
			printf("%s : adding %d background tiles (range %d-%d)\n", filename,
					bt, nbacktiles, nbacktiles + bt - 1);
		if (ft)
			printf("%s : adding %d foreground tiles (range %d-%d)\n", filename,
					ft, nforetiles, nforetiles + bt - 1);
		if (!ft && !bt)
			...
			if (ft)
				foretiles = (int *) realloc(foretiles,
						sizeof(int) * (nforetiles + ft));
			if (bt)
				backtiles = (int *) realloc(backtiles,
						sizeof(int) * (nbacktiles + bt));
...
void load_tiles(Cell *file_list) {
	...
	if (nbacktiles > old_bsize) {
		...
		memset(backtiles + old_bsize, -1,
				(nbacktiles - old_bsize) * sizeof(int));
	}

	if (nforetiles > old_fsize) {
		foretiles = (int *) realloc(foretiles, sizeof(int) * nforetiles);
		memset(foretiles + old_fsize, -1,
				(nforetiles - old_fsize) * sizeof(int));
	}
			...
			for (i = 0; i < sd->total; i++) {
				...
				case SPEC_BACKTILE:
					...
					if (sscanf(spe->name, "%d", &num)) {
						...
						backtiles[num] = cache.reg(fn, spe->name,
								SPEC_BACKTILE);
					}
					break;
				case SPEC_FORETILE:
					if (sscanf(spe->name, "%d", &num)) {
						...
						foretiles[num] = cache.reg(fn, spe->name,
								SPEC_FORETILE);
					}
					break;
				}
...
//  clouds      =      cache.reg(ff,"clouds",SPEC_IMAGE);
	...
	if (DEFINEDP(symbol_value(l_help_screens))) {
		...
		if (total_help_screens) {
			...
			int i = 0;
			for (; v; v = CDR(v), i++)
				help_screens[i] = cache.reg(ff, lstring_value(CAR(v)),
						SPEC_IMAGE);
		} else
	...
	if (DEFINEDP(symbol_value(l_title_screen)))
		title_screen = cache.reg_object(NULL,
				(LObject *) symbol_value(l_title_screen), SPEC_IMAGE, 1);
	else
		title_screen = -1;

	if (DEFINEDP(symbol_value(l_cdc_logo)))
		cdc_logo = cache.reg_object(NULL, (LObject *) symbol_value(l_cdc_logo),
				SPEC_IMAGE, 1);
	else
		cdc_logo = -1;
