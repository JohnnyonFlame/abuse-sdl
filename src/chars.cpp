...
char const *state_names[MAX_STATE] = { "dead", "dieing", "stopped",
		"start_run_jump", "run_jump", "run_jump_fall", "end_run_jump",
		"flinch_up", "flinch_down", "morph_pose", "running" };
...
char const *cflag_names[TOTAL_CFLAGS] = { "hurt_all", "is_weapon", "stoppable",
		"can_block", "hurtable", "pushable", "unlistable", "add_front",
		"cached_in", "need_cache_in", "unactive_shield" };

char const *ofun_names[TOTAL_OFUNS] = { "ai_fun", "move_fun", "draw_fun",
		"map_draw_fun", "damage_fun", "next_state_fun", "user_fun",
		"constructor", "reload_fun", "get_cache_list_fun", "type_change_fun" };
...
int character_type::add_state(void *symbol) // returns index into seq to use
	...
	if (num < ts && seq[num]) {
		((LObject *) symbol)->Print();
		lbreak(
				"symbol has been assigned value %d, but value already in use by state %s\n"
...
void character_type::add_var(void *symbol, void *name) {
	...
	if (DEFINEDP(s->value) && (item_type(s->value) != L_OBJECT_VAR)) {
		((LObject *) symbol)->Print();
		lbreak(
				"symbol already has a value, cannot instantiate an object varible");
		exit(0);
		...
		if (index < tiv) {
			if (vars[index]) {
				lbreak(
						...
						lstring_value(((LSymbol *) symbol)->GetName()), index,
						lstring_value(((LSymbol *) name)->GetName()),
	...
	for (; field; field = CDR(field)) {
		...
		if (f == l_abil) {
			...
			if (get_cflag(CFLAG_IS_WEAPON)) // if this is a weapon add to weapon array
					{
				total_weapons++;
				weapon_types = (int *) realloc(weapon_types,
						sizeof(int) * total_weapons);
			...
			while (!NILP(mf)) {
				...
				fields = (named_field **) realloc(fields,
						sizeof(named_field *) * total_fields);
