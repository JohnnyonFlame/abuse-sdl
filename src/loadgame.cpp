...
Jwindow *create_num_window(int mx, int total_saved, int lines,
		image **thumb_nails) {
	...
	for (i = 0; i < total_saved; i++, y += ih) {
		...
		buts[i] = new ico_button(x, y, ID_LOAD_GAME_NUMBER + n,
				save_buts[n * 3 + 0], save_buts[n * 3 + 0],
	...
	return wm->new_window(mx,
			yres / 2 - (Jwindow::top_border() + maxih * 5) / 2, -1, -1, buts[0]);
}
...
void get_savegame_name(char *buf) // buf should be at least 50 bytes
		{
	sprintf(buf, "save%04d.spe",
			(last_save_game_number++) % MAX_SAVE_GAMES + 1);
