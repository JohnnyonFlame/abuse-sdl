...
VolumeWindow::VolumeWindow() :
	...
	u_u = cache.reg(ff, "u_u", SPEC_IMAGE, 1), u_d = cache.reg(ff, "u_u",
			SPEC_IMAGE, 1), u_ua = cache.reg(ff, "u_ua", SPEC_IMAGE, 1), u_da =
			cache.reg(ff, "u_da", SPEC_IMAGE, 1), d_u = cache.reg(ff, "d_u",
			SPEC_IMAGE, 1), d_d = cache.reg(ff, "d_u", SPEC_IMAGE, 1), d_ua =
			cache.reg(ff, "d_ua", SPEC_IMAGE, 1), d_da = cache.reg(ff, "d_da",
			SPEC_IMAGE, 1), slider = cache.reg(ff, "volume_slide", SPEC_IMAGE,
			1);
			...
			new ico_button(10, 27, ID_SFX_DOWN, d_u, d_d, d_ua, d_da,
					new ico_button(21, 27, ID_SFX_UP, u_u, u_d, u_ua, u_da,
							new info_field(15, 42, 0, symbol_str("SFXv"),
									new ico_button(10, 72, ID_MUSIC_DOWN, d_u,
											d_d, d_ua, d_da,
											new ico_button(21, 72, ID_MUSIC_UP,
													u_u, u_d, u_ua, u_da,
													new info_field(10, 86, 0,
															symbol_str(
																	"MUSICv"),
															NULL)))))));
...
void VolumeWindow::draw_vol(int x1, int y1, int x2, int y2, int t, int max,
		int c1, int c2) {
