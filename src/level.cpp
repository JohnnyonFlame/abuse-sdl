...
game_object *level::attacker(game_object *who) {
	...
	for (; f; f = f->next) {
		if (f->focus) {
			int32_t tmp_d = abs(f->focus->x - who->x)
					+ abs(f->focus->y - who->y);
			if (tmp_d < d) {
				d = tmp_d;
				c = f->focus;
...
int level::add_actives(int32_t x1, int32_t y1, int32_t x2, int32_t y2) {
	...
	if (first_active)
		for (last_active = first_active; last_active->next_active; last_active =
	...
	for (; o; o = o->next) {
		if (!o->active) {
			int32_t xr = figures[o->otype]->rangex, yr =
					figures[o->otype]->rangey;

			if (o->x + xr >= x1 && o->x - xr <= x2 && o->y + yr >= y1
					&& o->y - yr <= y2) {
...
int level::add_drawables(int32_t x1, int32_t y1, int32_t x2, int32_t y2) {
	...
	for (; o; o = o->next) {
		if (ft || !o->active) {
			int32_t xr = figures[o->otype]->draw_rangex, yr =
					figures[o->otype]->draw_rangey;
...
void level::try_pushback(game_object *subject, game_object *target) {
	if (subject->pushable() && target->pushable() && subject->state != dead
			&& target->state != dead && subject->state != dieing
			&& target->state != dieing) {
		int b1 = subject->push_range(), b2 = target->push_range();
		if (abs(subject->x - target->x) < b1 + b2) {
			int32_t tmove = b1 + b2 - abs(subject->x - target->x), xv, yv = 0,
					xv2;
...
game_object *level::boundary_setback(game_object *subject, int32_t x1,
	...
	for (; t; t--, blist++) {
		target = *blist;
		if (target != subject
				&& (target->total_objects() == 0
						|| target->get_object(0) != subject)) {
			target->picture_space(tx1, ty1, tx2, ty2);
					...
					|| (y1 > ty2 && y2 > ty2) || (y1 < ty1 && y2 < ty1))) // are they semi/overlapping?
			{
				...
				while (iter-- > 0) {
					...
					// now check to see if (x1,y1-x2,y2) intercest with (xp1,yp1-xp2,yp2)
					if (*ins) {
						if (setback_intersect(x1, y1, x2, y2, xp1, yp1, xp2,
								yp2, 1))
							l = target;
					} else {
						if (setback_intersect(x1, y1, x2, y2, xp1, yp1, xp2,
...
game_object *level::all_boundary_setback(game_object *subject, int32_t x1,
		int32_t y1, int32_t &x2, int32_t &y2) {
	game_object *l = NULL;
	int32_t tx1, ty1, tx2, ty2, t_centerx;
	game_object *target = first_active;
		...
		if (!(dev & SUSPEND_MODE) || c) {
			...
			if (c) {
				...
				o->move(c->x_suggestion, c->y_suggestion,
						c->b1_suggestion | (c->b2_suggestion << 1)
								| (c->b3_suggestion << 2));
...
void level::set_size(int w, int h) {
	...
	int x, y, miny = (h < fg_height) ? h : fg_height, minx =
			(w < fg_width) ? w : fg_width;
			...
			if (fp->read_uint8() == RC_16) //  read type array, this should be type RC_16
			{
				...
				if (se) {
					...
					if (fp->read_uint8() == RC_16) //  read state array, this should be type RC_16
					{
						...
						for (i = 0; i < total_objs; i++, l = l->next) {
							character_state s =
									(character_state) s_remap[fp->read_uint16()];
							if (l->otype != 0xffff) {
				...
				for (; j < default_simple.total_vars(); j++) {
					...
					if (se) {
						...
						if (fp->read_uint8() != t)
							dprintf(
									"Warning : load level -> var '%s' size changed\n");
						else {
							game_object *f = first;
							for (; f; f = f->next) {
								switch (t) {
				...
				if (new_type < total_objects) // make sure old object still exsist
					...
					for (; k < figures[new_type]->ts; k++) {
						if (figures[new_type]->seq[k]
								&& !strcmp(
										lstring_value(
												((LSymbol *) figures[new_type]->seq_syms[k])->GetName()),
										old_name))
							*(s_remap[i] + j) = k;
					}
				}
					...
					if (new_type != 0xffff) // make sure old object still exsist
						...
						for (; k < figures[new_type]->tiv; k++) {
							if (figures[new_type]->vars[k]) {
								if (!strcmp(
										lstring_value(
												((LSymbol *) figures[new_type]->vars[k])->GetName()),
										old_name))
									*(v_remap[i] + j) =
											figures[new_type]->var_index[k];
							}
						}
					}
				}
				...
				if (fp->read_uint8() == RC_16) //  read type array, this should be type RC_16
				{
						...
						if (fp->read_uint8() == RC_16) //  read state array, this should be type RC_16
						{
							...
							for (i = 0; i < total_objs; i++, l = l->next) {
								...
								if (l->otype == 0xffff)
									...
									character_state s =
											(character_state) (*(s_remap[o_backmap[l->otype]]
													+ st));
									if (l->has_sequence((character_state) s))
										l->state = s;
									else
					...
					for (; j < default_simple.total_vars(); j++) {
						...
						if (se) {
							...
							dprintf(
									"Warning : load level -> no previous var %s\n",
									default_simple.var_name(j));
					}
				}
			}
...
bFILE *level::create_dir(char *filename, int save_all, object_node *save_list,
		object_node *exclude_list) {
	spec_directory sd;
	sd.add_by_hand(
			new spec_entry(SPEC_DATA_ARRAY,
					"Copyright 1995 Crack dot Com, All Rights reserved", NULL,
					0, 0));
	if (first_name)
		sd.add_by_hand(
				new spec_entry(SPEC_DATA_ARRAY, "first name", NULL,
						strlen(first_name) + 2, 0));

	sd.add_by_hand(
			new spec_entry(SPEC_GRUE_FGMAP, "fgmap", NULL,
					4 + 4 + fg_width * fg_height * 2, 0));
	sd.add_by_hand(
			new spec_entry(SPEC_GRUE_BGMAP, "bgmap", NULL,
					4 + 4 + bg_width * bg_height * 2, 0));
	sd.add_by_hand(
			new spec_entry(SPEC_DATA_ARRAY, "bg_scroll_rate", NULL, 1 + 4 * 4,
					0));
	...
	sd.add_by_hand(
			new spec_entry(SPEC_DATA_ARRAY, "area_list.v1", NULL,
					1 + ta * (4 * 11) + 4, 0));

	sd.add_by_hand(
			new spec_entry(SPEC_DATA_ARRAY, "tick_counter", NULL, 1 + 4, 0));

	// how many object types are we goint to save, use a short to specify how many
	sd.add_by_hand(
			new spec_entry(SPEC_DATA_ARRAY, "object_descripitions", NULL, 2,
					0));
	...
	sd.add_by_hand(
			new spec_entry(SPEC_DATA_ARRAY, "describe_names", NULL, size, 0));
			...
			if (figures[i]->seq[j])
				size +=
						1
								+ strlen(
										lstring_value(
												((LSymbol *) figures[i]->seq_syms[j])->GetName()))
								+ 1;
	}
	sd.add_by_hand(
			new spec_entry(SPEC_DATA_ARRAY, "describe_states", NULL, size, 0));
			...
			if (figures[i]->vars[j])
				size +=
						1
								+ strlen(
										lstring_value(
												((LSymbol *) figures[i]->vars[j])->GetName()))
								+ 1;
	}
	sd.add_by_hand(
			new spec_entry(SPEC_DATA_ARRAY, "describe_lvars", NULL, size, 0));
	...
	sd.add_by_hand(
			new spec_entry(SPEC_DATA_ARRAY, "state", NULL, 1 + 2 * t, 0));
	...
	for (i = 0; i < TOTAL_OBJECT_VARS; i++)
		sd.add_by_hand(
				new spec_entry(SPEC_DATA_ARRAY, object_descriptions[i].name,
						NULL, 1 + RC_type_size(object_descriptions[i].type) * t,
						0));

	add_light_spec(&sd, Name);

	sd.add_by_hand(
			new spec_entry(SPEC_DATA_ARRAY, "object_links", NULL,
					1 + 4 + total_object_links(save_list) * 8, 0));
	sd.add_by_hand(
			new spec_entry(SPEC_DATA_ARRAY, "light_links", NULL,
					1 + 4 + total_light_links(save_list) * 8, 0));
	...
	if (save_all) {
		...
		for (; v; v = v->next)
			t++;
		sd.add_by_hand(
				new spec_entry(SPEC_DATA_ARRAY, "player_info", NULL, t * 4 + 4,
						0));
		...
		for (; i < tv; i++)
			sd.add_by_hand(
					new spec_entry(SPEC_DATA_ARRAY, get_view_var_name(i), NULL,
							1 + 4 * t, 0));
		sd.add_by_hand(
				new spec_entry(SPEC_DATA_ARRAY, "random_start", NULL, 5, 0));

		sd.add_by_hand(
				new spec_entry(SPEC_DATA_ARRAY, "weapon_array", NULL,
						1 + 4 + total_weapons * 4 * t, 0));

		int name_len = 0;
		for (v = player_list; v; v = v->next)
			name_len += strlen(v->name) + 2;
		sd.add_by_hand(
				new spec_entry(SPEC_DATA_ARRAY, "player_names", NULL, name_len,
						0));

		sd.add_by_hand(
				new spec_entry(SPEC_IMAGE, "thumb nail", NULL,
						4 + 160 * (100 + wm->font()->height() * 2), 0));
	}
...
void level::write_thumb_nail(bFILE *fp, image *im) {
	...
	if (first_name)
		wm->font()->put_string(i,
				80 - strlen(first_name) * wm->font()->width() / 2, 100,
				first_name);
	...
	wm->font()->put_string(i, 80 - strlen(buf) * wm->font()->width() / 2,
			100 + wm->font()->height(), buf);
...
int level::load_player_info(bFILE *fp, spec_directory *sd,
		object_node *save_list) {
	int ret;
	...
	if (se) {
		...
		for (; i < total_players; i++) {
			game_object *o = number_to_object_in_list(fp->read_uint32(),
					save_list);
			v = new view(o, NULL, 0);
			if (o)
				o->set_controller(v);
...
void level::write_links(bFILE *fp, object_node *save_list,
		object_node *exclude_list) {
	fp->write_uint8(RC_32);
	...
	for (; o; o = o->next, x++) {
		...
		for (; i < o->me->total_objects(); i++) {
			fp->write_uint32(x);
			int32_t x = object_to_number_in_list(o->me->get_object(i),
					save_list);
			if (x)
				fp->write_uint32(x);
			else
				// save links to excluded items as negative
				fp->write_uint32(
						(int32_t) (-(object_to_number_in_list(o->me,
								exclude_list))));
		}
	}
...
void level::load_links(bFILE *fp, spec_directory *sd, object_node *save_list,
		object_node *exclude_list) {
...
void level::write_cache_prof_info() {
	if (cache.prof_is_on()) {
		...
		spec_directory sd;
		sd.add_by_hand(
				new spec_entry(SPEC_DATA_ARRAY, "cache profile info", NULL,
						cache.prof_size(), 0));
		sd.calc_offsets();
...
void level::load_cache_info(spec_directory *sd, bFILE *fp) {
	if (!DEFINEDP(symbol_value(l_empty_cache))
			|| !symbol_value(l_empty_cache)) {
		char pf_name[100];
		if (first_name)
...
void level::foreground_intersect(int32_t x1, int32_t y1, int32_t &x2,
		int32_t &y2) {
	/*  if (x1==x2)
	...
	int32_t tl = the_game->ftile_width(), th = the_game->ftile_height(), j, xp1,
			yp1, xp2, yp2, // starting and ending points of block line segment
			swap; // temp var
		...
		for (by = blocky1; by <= blocky2; by++) {
			...
			if (block > BLACK) // don't check BLACK, should be no points in it
			{
				...
				for (j = 0; j < total - 1; j++, ins++) {
					...
					if (*ins)
						setback_intersect(x1, y1, x2, y2, xp1, yp1, xp2, yp2,
								1);
					else
						setback_intersect(x1, y1, x2, y2, xp1, yp1, xp2, yp2,
								-1);
...
void level::vforeground_intersect(int32_t x1, int32_t y1, int32_t &y2) {
	...
	for (by = blocky1; by <= blocky2; by++, y1 -= f_hi, y2 -= f_hi, y_addback +=
			f_hi) {
		block = the_game->get_map_fg(bx, by);
...
//    int32_t xo=bx*tl,yo=by*th;
		for (j = 0; j < total - 1; j++, ins++) {
			...
			int32_t oy2 = y2;
			if (*ins)
				setback_intersect(checkx, y1, checkx, y2, xp1, yp1, xp2, yp2,
						1);
			else
				setback_intersect(checkx, y1, checkx, y2, xp1, yp1, xp2, yp2,
						-1);
			...
			if (o->try_move(o->x, o->y, xv, yv, 3) == by_who) {
				...
				if (xv == 0 && yv == 0) {
					if (o->state != dead && o->state != dieing)
						o->do_damage(by_who->current_figure()->hit_damage,
								by_who, o->x, o->y, 0, 0);
...
int level::push_characters(game_object *by_who, int xamount, int yamount) {
	...
	game_object *o = first_active;
	for (; o; o = o->next_active) {
		if ((o->is_playable() || o->pushable()) && o->state != dieing
				&& o->state != dead) {
			xv = -xamount;
			yv = -yamount;
			int32_t tvx, tvy;
			...
			if (yv > 0)
				...
				tvy = 0;
			if (o->try_move(o->x + xamount, o->y + yamount, tvx, tvy, 3)
					== by_who) {
				xv = (xamount - tvx);
				yv = (yamount - tvy);
				o->try_move(o->x, o->y, xv, yv, 3);
...
void level::hurt_radius(int32_t x, int32_t y, int32_t r, int32_t m,
		game_object *from, game_object *exclude, int max_push) {
	if (r < 1)
...
game_object *level::get_random_start(int min_player_dist, view *exclude) {
	...
	do {
		...
		for (; v; v = v->next) {
			if (v != exclude) {
				int32_t cx = abs(v->x_center() - o->x), cy = abs(
						v->y_center() - o->y), d;
				if (cx < cy)
					d = cx + cy - (cx >> 1);
				else
...
void level::add_attacker(game_object *who) {
	if (attack_total >= attack_list_size) // see if we need to grow the list size..
			{
		attack_list_size++;
		attack_list = (game_object **) realloc(attack_list,
				sizeof(game_object *) * attack_list_size);
	}
...
void level::add_target(game_object *who) {
	if (target_total >= target_list_size) // see if we need to grow the list size..
			{
		target_list_size++;
		target_list = (game_object **) realloc(target_list,
				sizeof(game_object *) * target_list_size);
	}
...
void level::add_block(game_object *who) {
	if (block_total >= block_list_size) // see if we need to grow the list size..
			{
		block_list_size++;
		block_list = (game_object **) realloc(block_list,
				sizeof(game_object *) * block_list_size);
	}
...
void level::add_all_block(game_object *who) {
	if (all_block_total >= all_block_list_size) // see if we need to grow the list size..
			{
		all_block_list_size++;
		all_block_list = (game_object **) realloc(all_block_list,
				sizeof(game_object *) * all_block_list_size);
	}
	all_block_list[all_block_total] = who;
	all_block_total++;
}

game_object *level::find_object_in_area(int32_t x, int32_t y, int32_t x1,
		int32_t y1, int32_t x2, int32_t y2, Cell *list, game_object *exclude) {
...
game_object *level::find_object_in_angle(int32_t x, int32_t y,
		int32_t start_angle, int32_t end_angle, void *list,
		game_object *exclude) {
	game_object *closest = NULL;
	int32_t closest_distance = 0xfffffff, distance, xo, yo;
	game_object *o = first_active;
	...
	if (fp) {
		...
		for (; o; o = o->next) {
			fprintf(fp, "%3d %s %4ld %4ld %4ld %4ld %04d\n", i++,
					object_names[o->otype], (long) o->x, (long) o->y,
...
area_controller::area_controller(int32_t X, int32_t Y, int32_t W, int32_t H,
