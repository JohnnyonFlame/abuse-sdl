			...
			for (j = 0, point2 = data; j < tot - 1; j++, point2 += 2) {
				if (skip_next)
					...
					if (j != i) // make sure we are not looking at ourself
						...
						if ((checkx >= xp1 && checkx <= xp2)
								|| (checkx >= xp2 && checkx <= xp1)) {
							if (check_down && (yp1 > miny && yp2 > miny))
								segs_down++;
							if (checkx == xp2)
								skip_next = 1;
						} else if ((checky >= yp1 && checky <= yp2)
								|| (checky >= yp2 && checky <= yp1)) {
...
foretile::foretile(bFILE *fp) {
	...
	int r[AUTOTILE_WIDTH * AUTOTILE_HEIGHT],
			g[AUTOTILE_WIDTH * AUTOTILE_HEIGHT], b[AUTOTILE_WIDTH
					* AUTOTILE_HEIGHT], t[AUTOTILE_WIDTH * AUTOTILE_HEIGHT];
	...
	for (y = 0; y < h; y++) {
		sl = img->scan_line(y);
		for (x = 0; x < w; x++, sl++) {
			l = (y * AUTOTILE_HEIGHT / h) * AUTOTILE_WIDTH
					+ x * AUTOTILE_WIDTH / w;
			r[l] += pal->red(*sl);
			g[l] += pal->green(*sl);
			b[l] += pal->blue(*sl);
