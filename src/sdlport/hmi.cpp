...
static void remember_note_off_event(uint32_t time, uint8_t cmd, uint8_t note) {
	...
	qsort(note_off_events, MAX_NOTE_OFF_EVENTS, sizeof(NoteOffEvent),
			compare_times);
}
...
static void convert_hmi_track(uint8_t* input, uint32_t input_size,
		uint8_t* &output) {
	int done = 0;
	...
	uint8_t track_header[] = { 0x4D, 0x54, 0x72, 0x6B, 0x00, 0x00, 0x00, 0x00,
			0x00 };
	...
	while (!done) {
		...
		case 0x90: // Note on, non-standard, HMI files specify the duration as a third param
			*output++ = current_value;
			*output++ = *input++;
			remember_note_off_event(current_time + read_time_value(input),
					current_command, current_value);
			break;
	...
	write_big_endian_number((uint32_t) (output - start_of_buffer - 8),
			&start_of_buffer[4]);
}
...
uint8_t* load_hmi(char const *filename, uint32_t &data_size) {
	...
	uint8_t midi_header[] = { 0x4D, 0x54, 0x68, 0x64, 0x00, 0x00, 0x00, 0x06,
			0x00, 0x01, 0x00, (num_tracks + 1), 0x00, 0xC0 };
	memcpy(output_buffer_ptr, midi_header, 14);
	output_buffer_ptr += 14;

	// Write additional first track with bpm info
	uint8_t bpm_track[] = { 0x4D, 0x54, 0x72, 0x6B, 0x00, 0x00, 0x00, 0x0B,
			0x00, 0xFF, 0x51, 0x03, 0x18, 0x7F, 0xFF, 0x00, 0xFF, 0x2F, 0x00 };
	memcpy(output_buffer_ptr, bpm_track, sizeof(bpm_track));
	output_buffer_ptr += sizeof(bpm_track);

	for (int i = 0; i < num_tracks; i++) {
		uint32_t trackposition = get_int_from_buffer(
				&input_buffer[offset_tracks + i * (sizeof(uint32_t))]);
		uint32_t tracksize;
		if (i == num_tracks - 1)
			tracksize = buffersize - trackposition;
		else
			tracksize = get_int_from_buffer(
					&input_buffer[offset_tracks + (i + 1) * (sizeof(uint32_t))])
					- trackposition;

		convert_hmi_track(&input_buffer[trackposition], tracksize,
				output_buffer_ptr);
	}
