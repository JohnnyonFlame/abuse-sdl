...
void set_mode(int mode, int argc, char **argv) {
	...
	window = SDL_SetVideoMode(flags.xres, flags.yres, 8,
			vidFlags | SDL_ANYFORMAT);
	if (window == NULL) {
	...
	surface = SDL_CreateRGBSurface(SDL_SWSURFACE, window->w, window->h, 8, 0xff,
			0xff, 0xff, 0xff);
	if (surface == NULL) {
...
void put_part_image(image *im, int x, int y, int x1, int y1, int x2, int y2) {
	...
	dpixel += (dstrect.x + ((dstrect.y) * surface->w))
	...
	} else // sw scaling
	{
		...
		for (ii = 0; ii < dstrect.h; ii++) {
			...
			for (jj = 0; jj < dstrect.w; jj++) {
				memcpy(dpixel,
						im->scan_line((srcy >> 16))
								+ ((srcx >> 16) * surface->format->BytesPerPixel),
						surface->format->BytesPerPixel);
