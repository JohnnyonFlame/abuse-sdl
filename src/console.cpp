...
void console::show() {
	if (!con_win) {
		con_win = wm->new_window(lastx, lasty, screen_w(), screen_h(), NULL,
				name);
		redraw();
		con_win->screen->SetClip(con_win->x1(), con_win->y1(),
				con_win->x2() + 1, con_win->y2() + 1);
	}
}
...
void console::draw_cursor() {
	if (con_win)
		fnt->put_char(con_win->screen, cx * fnt->width() + wx(),
				cy * fnt->height() + wy(), '_');
}
