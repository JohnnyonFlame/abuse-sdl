...
class ico_button: public ifield {
	...
	int is_act;
public:
	ico_button(int X, int Y, int ID, int Up, int down, int upa, int downa,
			ifield *Next, int act_id = -1, char const *help_key = NULL);
...
class ico_switch_button: public ifield {
	...
	int act;
public:
	ico_switch_button(int X, int Y, int ID, int start_on, ifield *butts,
			ifield *Next);
	virtual void area(int &x1, int &y1, int &x2, int &y2);
	virtual void draw_first(image *screen) {
