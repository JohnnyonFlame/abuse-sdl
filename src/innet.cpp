...
int net_init(int argc, char **argv) {
	...
	for (i = 1; i < argc; i++) {
		if (!strcmp(argv[i], "-nonet")) {
			...
			if (i == argc - 1 || !sscanf(argv[i + 1], "%d", &x) || x < 1
					|| x > 0x7fff) {
				fprintf(stderr,
						"Net: Bad value following -port, use 1..32000\n");
				return 0;
			} else {
				main_net_cfg->port = x;
			}
	...
	for (; n; n = n->next) // show a list of usables, just to be cute
		...
		fprintf(stderr, "Protocol %s : ",
				n->installed() ? "Installed" : "Not_installed");
		fprintf(stderr, "%s\n", n->name());
		if (n->installed()) {
			total_usable++;
			usable = n;
...
int NF_set_file_server(net_address *addr) {
	if (prot) {
		...
		net_socket *sock = prot->connect_to_server(addr,
		...
		if ((sock->write(&cmd, 1) != 1
				&& printf("set_file_server::writefailed\n"))
				|| (sock->read(&cmd, 1) != 1
				...
				if (new_sock) {
					...
					if (new_sock->read(&client_type, 1) != 1) {
						...
						case CLIENT_LSF_WAITER: // wants to know which .lsp file to start with
						{
							...
							break;
						default: {
							if (game_face->add_client(client_type, new_sock,
									addr) == 0) // ask server or client to add new client
									{
								delete addr;
								delete new_sock;
							}
...
int get_remote_lsf(net_address *addr, char *filename) // filename should be 256 bytes
	...
	if (prot) {
		...
		if (sock->write(&ctype, 1) != 1 || sock->read(&len, 1) != 1 || len == 0
...
int request_server_entry() {
	if (prot && main_net_cfg) {
		...
		net_socket *sock = prot->connect_to_server(net_server,
				net_socket::SOCKET_SECURE);
		if (!sock) {
		...
		if (net_server) {
			...
			Jwindow *j = wm->new_window(0, yres / 2, -1, -1,
					new info_field(0, 0, 0, symbol_str("resync"),
							new button(0, wm->font()->height() + 5,
									ID_NET_DISCONNECT, symbol_str("slack"),
									NULL)), symbol_str("hold!"));
				...
				if (wm->event_waiting()) {
					...
					do {
						...
						if (ev.type == EV_MESSAGE
								&& ev.message.id == ID_NET_DISCONNECT) {
...
int get_inputs_from_server(unsigned char *buf) {
		...
		while (base->input_state != INPUT_PROCESSING) {
			...
			if (now.diff_time(&start) > 0.05) {
				...
				if (total_retry == 12000) // 2 minutes and nothing
						{
					abort = wm->new_window(0, yres / 2, -1,
							wm->font()->height() * 4,
	...
	memcpy(base->last_packet.data, base->packet.data,
