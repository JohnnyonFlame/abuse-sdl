...
void status_bar::redraw(image *screen) {
	...
	if (total_weapons) {
		...
		int sb_w = (small_render ? sb->Size().x * 2 : sb->Size().x), sb_h = (
				small_render ? sb->Size().y * 2 : sb->Size().y);
		...
		int ww =
				small_render ?
						cache.img(bweap[0])->Size().x * 2 :
						cache.img(bweap[0])->Size().x;
		int wh =
				small_render ?
						cache.img(bweap[0])->Size().y * 2 :
						cache.img(bweap[0])->Size().y;

		// numpad y offset
		int np_yo = small_render ? 42 : 21;
		int np_w =
				small_render ?
						cache.img(sbar_numpad)->Size().x * 2 :
						cache.img(sbar_numpad)->Size().x;
		int np_h =
				small_render ?
						cache.img(sbar_numpad)->Size().y * 2 :
						cache.img(sbar_numpad)->Size().y;

		// selection bar width * height
		int sel_w =
				small_render ?
						cache.img(sbar_select)->Size().x * 2 :
						cache.img(sbar_select)->Size().x;
		int sel_h =
				small_render ?
						cache.img(sbar_select)->Size().y * 2 :
						cache.img(sbar_select)->Size().y;

		int sel_off = small_render ? 8 : 4;
		scale_put(sb, screen, sx, sy, sb_w, sb_h);

		if (v->focus)
			draw_num(screen, sx + (small_render ? 17 * 2 : 17),
					sy + (small_render ? 11 * 2 : 11), v->focus->hp(), bnum);
		...
		for (i = 0; i < t; i++, x_on += wa, ammo_x += wa) {
			if (v->has_weapon(i)) {
				if (v->current_weapon == i)
					scale_put_trans(cache.img(bweap[i]), screen, x_on, sy, ww,
							wh);
				else
					scale_put_trans(cache.img(dweap[i]), screen, x_on, sy, ww,
							wh);

				scale_put_trans(cache.img(sbar_numpad), screen, x_on - 2,
						sy + np_yo, np_w, np_h);

				if (v->current_weapon == i)
					draw_num(screen, ammo_x, ammo_y, v->weapon_total(i),
							bnum + 20);
				else
					draw_num(screen, ammo_x, ammo_y, v->weapon_total(i),
							bnum + 10);
				...
				if (i == icon_in_selection)
					scale_put_trans(cache.img(sbar_select), screen,
							x_on + sel_off, sy, sel_w, sel_h);
			}
		}
	}
}
...
void status_bar::draw_health(image *screen, int amount) {
	if (total_weapons) {
		int x1, y1, x2, y2;
		area(x1, y1, x2, y2);
		draw_num(screen, x1 + (small_render ? 17 * 2 : 17),
				y1 + (small_render ? 11 * 2 : 11), amount, bnum);
	}
}

void status_bar::draw_ammo(image *screen, int weapon_num, int amount,
		int light) {
	if (total_weapons) {
		int x1, y1, x2, y2;
		area(x1, y1, x2, y2);
		draw_num(screen,
				x1
						+ (small_render ?
								52 * 2 + weapon_num * 34 * 2 :
								52 + weapon_num * 34),
				y1 + (small_render ? 25 * 2 : 25), amount,
				bnum + (light ? 20 : 10));
	}
}
...
void status_bar::draw_update() {
	if (total_weapons && v) {
		if (DEFINEDP(symbol_value(l_mouse_can_switch))
				&& symbol_value(l_mouse_can_switch) && mouse_in_area()) {
			if ((current_level->tick_counter() & 4) == 0)
				wm->set_mouse_shape(cache.img(c_mouse1)->copy(), 4, 4);
			else
				wm->set_mouse_shape(cache.img(c_mouse2)->copy(), 4, 4);
			changed_cursor = 1;
...
void status_bar::step() {
	...
	if (sy1 < view_y2) // tell view to shrink if it is overlapping the status bar
		...
		v->suggest.cx2 = v->cx2;
		v->suggest.cy2 =
				small_render ? (sy1 - v->cy1 - 2) / 2 + v->cy1 : sy1 - 2;
	}
