...
void morph_char::draw(game_object *who, view *v) {
	if (fleft) {
		int32_t rx, ry;
		the_game->game_to_mouse(who->x - (cx >> 16), who->y - (cy >> 16), v, rx,
				ry);
...
morph_char::morph_char(game_object *who, int to_type, void (*stat_fun)(int),
		int anneal, int frames) {
	mor = NULL;
	...
	if (!t1->has_sequence(morph_pose) || t1->morph_mask < 0
		...
		if (sm->t) {
			delete h1;
			delete h2;
			figure *f1 = t1->get_sequence(morph_pose)->get_figure(0), *f2 =
					t2->get_sequence(morph_pose)->get_figure(0);
