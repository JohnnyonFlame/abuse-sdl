...
void director::wait(void *arg) {
	...
	int done = 0;
	LSymbol *pan_symbol = LSymbol::FindOrCreate("pan"), *text_symbol =
			LSymbol::FindOrCreate("text");
	...
	do {
		...
		if (text) {
			if (text_draw(text_y, the_game->first_view->cx1 + tleft,
				...
				text = NULL;
			if (text_time) {
				if ((int) (cur_time.diff_time(text_time) * 1000)
						> scroll_speed) {
					text_y += text_step;
					delete text_time;
					if (text)
