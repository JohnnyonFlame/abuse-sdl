...
public:
	...
	game_object **touch;
	collide_patch *next;
	collide_patch(int32_t X1, int32_t Y1, int32_t X2, int32_t Y2,
			collide_patch *Next) {
		x1 = X1;
		y1 = Y1;
		x2 = X2;
	...
	void add_collide(int32_t X1, int32_t Y1, int32_t X2, int32_t Y2,
			game_object *who);
...
void add_collide(collide_patch *&first, int32_t x1, int32_t y1, int32_t x2,
		int32_t y2, game_object *who) {
	collide_patch *next;
	for (collide_patch *p = first; p; p = next) {
		...
		if (x1 >= p->x1 && y1 >= p->y1 && x2 <= p->x2 && y2 <= p->y2) {
			...
			p->total++;
			p->touch = (game_object **) realloc(p->touch,
					sizeof(game_object *) * p->total);
		...
		if (!(x2 < p->x1 || y2 < p->y1 || x1 > p->x2 || y1 > p->y2)) {
			int ax1, ay1, ax2, ay2;
			if (x1 < p->x1) {
				add_collide(first, x1, Max(y1, p->y1), p->x1 - 1,
						Min(y2, p->y2), who);
				ax1 = p->x1;
			} else
				ax1 = x1;
			...
			if (x2 > p->x2) {
				add_collide(first, p->x2 + 1, Max(y1, p->y1), x2,
						Min(y2, p->y2), who);
				ax2 = p->x2;
			} else
				ax2 = x2;
...
void level::check_collisions() {
	...
	int32_t sx1, sy1, sx2, sy2, tx1, ty1, tx2, ty2, hitx = 0, hity = 0,
			t_centerx;
	...
	for (int l = 0; l < attack_total; l++) {
			...
			if (!(sx2 < tx1 || sy2 < ty1 || sx1 > tx2 || sy1 > ty2)) // check to see if picture spaces collide
			{
				...
				if (subject->can_hurt(target)) // see if we can hurt him before calculating
					...
					for (i = (int) s_hit->tot - 1; i > 0 && !rec; i--) {
						for (t_dat = t_damage->data, j = (int) t_damage->tot - 1;
								j > 0 && !rec; j--) {
							...
							setback_intersect(x1, y1, x2, y2, xp1, yp1, xp2,
									yp2, 0);
		...
		if (rec) {
			rec->do_damage((int) subject->current_figure()->hit_damage, subject,
					hitx, hity, 0, 0);
