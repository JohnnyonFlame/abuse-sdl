...
static int ant_congestion(game_object *o) {
	for (game_object *d = current_level->first_active_object(); d;
			d = d->next_active) {
		if (d->otype == o->otype && abs(o->x - d->x) < 30
				&& abs(o->x - d->y) < 20)
			return 1;
	}
	return 0;
...
static int can_hit_player(game_object *o, game_object *b) {
	return can_see(o, o->x + (o->direction > 0 ? 15 : -15), o->y - 15, b->x,
			b->y - 15);
}
...
static void fire_at_player(game_object *o, game_object *b) {
	...
	if (can_see(o, o->x, o->y, firex, firey)
			&& can_see(o, firex, firey, playerx, playery)) {
...
void *ant_ai() {
	...
	case ANT_RUNNING: {
		...
		if (!ant_dodge(o)) {
			if ((o->x > b->x && o->direction == -1)
					|| (o->x < b->x && o->direction == 1)) {
				o->next_picture();
				if ((jrand() % 4) == 0 && abs(o->x - b->x) < 180
						&& abs(o->y - b->y) < 100 && can_hit_player(o, b)) {
					o->set_state((character_state) S_weapon_fire);
					o->set_aistate(ANT_FIRE);
				} else if (abs(o->x - b->x) < 100 && abs(o->y - b->y) < 10
						&& (jrand() % 4) == 0)
	...
	case ANT_FIRE: {
		if (!ant_dodge(o)) {
			if (o->state == S_fire_wait) {
				if (!o->next_picture()
						|| symbol_value(l_difficulty) == l_extreme) {
					if (player_list->next)
						b = current_level->attacker(current_object);
					else
						b = player_list->focus;
...
//      o->set_yvel(o->yvel()+1);
	...
	case ANT_ROOF_WALK: {
		...
		if (((jrand() % 8) == 0 && abs(o->x - b->x) < 10 && o->y < b->y)
			...
			if ((o->x > b->x && o->direction > 0)
					|| (o->x < b->x && o->direction < 0))
				...
				if (can_see(o, o->x, o->y - 31, o->x + speed, o->y - 31)
						&& !can_see(o, o->x + speed, o->y - 31, o->x + speed,
								o->y - 32)) {
					o->x += speed;
					if (!o->next_picture())
						o->set_state((character_state) S_top_walk);
...
void show_stats() {
	if (current_level) {
		...
		if (isdigit(dig1) && isdigit(dig2)) {
			if (dig1 != '0')
				sprintf(msg, "%s : %c%c", symbol_str("lev_complete"), dig1,
						dig2);
			else
				sprintf(msg, "%s : %c", symbol_str("lev_complete"), dig2);
		} else
			sprintf(msg, "%s : %s", symbol_str("lev_complete"),
					current_level->original_name());
