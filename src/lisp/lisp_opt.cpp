...
void *colon_initial_contents, *colon_initial_element, *eq_symbol, *zero_symbol,
		*eq0_symbol, *load_warning;
	...
	if (list) {
		if (CAR(list) == if_symbol) {
			...
			if (lcar(list) == eq_symbol && (lcar(lcdr(list)) == zero_symbol)) //  simplify (eq 0 x) -> (eq0 x)
				...
				return_val = comp_optimize(ret);
			} else if (lcar(eval1) == progn_symbol
					&& (eval2 == NULL || item_type(eval2) != L_CONS_CELL)) {
