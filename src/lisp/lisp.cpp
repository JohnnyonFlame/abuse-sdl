...
LSysFunction *new_lisp_sys_function(int min_args, int max_args,
		int fun_number) {
...
LSysFunction *new_user_lisp_function(int min_args, int max_args,
		int fun_number) {
	LSysFunction *ls = new_lisp_sys_function(min_args, max_args, fun_number);
...
void *nth(int num, void *list) {
	if (num < 0) {
		lbreak(
				"NTH: %d is not a nonnegative fixnum and therefore not a valid index\n",
				num);
		exit(1);
	}
...
long lfixed_point_value(void *c) {
	switch (item_type(c)) {
	case L_NUMBER:
		return ((LNumber *) c)->num << 16;
		break;
	case L_FIXED_POINT:
		return (((LFixedPoint *) c)->x);
		break;
	default: {
		((LObject *) c)->Print();
		lbreak(" is not a number\n");
...
int32_t lisp_cos(int32_t x) {
	...
	if (x < 0)
		return sin_table[FIXED_TRIG_SIZE + x];
	else
		return sin_table[x];
}
...
void *pairlis(void *list1, void *list2, void *list3) {
	if (item_type(list1) != (ltype) L_CONS_CELL
			|| item_type(list1) != item_type(list2))
		return NULL;
...
LSymbol *add_sys_function(char const *name, short min_args, short max_args,
		short number) {
	...
	if (s->function != l_undefined) {
		...
		s->function = new_lisp_sys_function(min_args, max_args, number);
	return s;
}
...
LSymbol *add_c_object(void *symbol, int index) {
	...
	if (s->value != l_undefined) {
		...
		s->value = LObjectVar::Create(index);
	return NULL;
}

LSymbol *add_c_function(char const *name, short min_args, short max_args,
		short number) {
	...
	if (s->function != l_undefined) {
		...
		s->function = new_lisp_c_function(min_args, max_args, number);
	return s;
}

LSymbol *add_c_bool_fun(char const *name, short min_args, short max_args,
		short number) {
	total_user_functions++;
	...
	if (s->function != l_undefined) {
		...
		s->function = new_lisp_c_bool(min_args, max_args, number);
	return s;
}

LSymbol *add_lisp_function(char const *name, short min_args, short max_args,
		short number) {
	total_user_functions++;
	need_perm_space("add_c_bool_fun");
	LSymbol *s = LSymbol::FindOrCreate(name);
	if (s->function != l_undefined) {
		...
		s->function = new_user_lisp_function(min_args, max_args, number);
	return s;
}
...
void LObject::Print() {
	...
	case L_FIXED_POINT:
		sprintf(buf, "%g",
				(lfixed_point_value(this) >> 16)
						+ ((lfixed_point_value(this) & 0xffff))
								/ (double) 0x10000);
		lprint_string(buf);
		break;
	case L_CHARACTER:
...
LObject *LSymbol::EvalFunction(void *arg_list) {
	...
	case L_SYS_FUNCTION:
		...
		break;
	case L_L_FUNCTION:
		ret = (LObject *) l_caller(((LSysFunction *) fun)->fun_number,
				arg_list);
		break;
	case L_USER_FUNCTION:
		return EvalUserFunction((LList *) arg_list);
	...
	case L_C_BOOL: {
		...
		if (t == L_C_FUNCTION)
			ret = LNumber::Create(
					c_caller(((LSysFunction *) fun)->fun_number, first));
		...
		if (!elements)
			...
			for (i = 0; i < elements; i++) {
				...
				case L_CONS_CELL: {
					...
					break;
				case L_STRING: {
					memcpy(s, lstring_value(str_eval[i]),
...
LObject *LSysFunction::EvalFunction(LList *arg_list) {
	...
	case SYS_FUNC_SETF: {
		...
		case L_SYMBOL:
			...
			case L_NUMBER:
				...
				break;
			case L_OBJECT_VAR:
				l_obj_set(((LObjectVar *) (((LSymbol *) i)->value))->index,
						set_to);
				break;
			default:
	...
	case SYS_FUNC_LET: {
		...
		while (var_list) {
			...
			((LSymbol *) var_name)->SetValue(
					(LObject *) l_user_stack.sdata[cur_stack++]);
			var_list = CDR(var_list);
		}
	...
	case SYS_FUNC_DEFUN: {
		...
		LUserFunction *ufun = new_lisp_user_function(
				(LList *) lcar(lcdr(arg_list)), (LList *) block_list);
	...
	case SYS_FUNC_COMPILE_FILE: {
		...
		if (fun_number == SYS_FUNC_LOCAL_LOAD) {
			...
			if (strcmp(st, "gamma.lsp") == 0) {
				char *gammapath;
				gammapath = (char *) malloc(
						strlen(get_save_filename_prefix()) + 9 + 1);
	...
	case SYS_FUNC_RESIZE_PERM:
		...
		break;
	case SYS_FUNC_COS:
		ret = LFixedPoint::Create(
				lisp_cos(lnumber_value(CAR(arg_list)->Eval())));
		break;
	case SYS_FUNC_SIN:
		ret = LFixedPoint::Create(
				lisp_sin(lnumber_value(CAR(arg_list)->Eval())));
		break;
	case SYS_FUNC_ATAN2: {
	...
	case SYS_FUNC_DO: {
		...
		for (int i = 0; !i;) // set i to 1 when terminate conditions are met
			...
			if (!i) {
				...
				for (init_var = CAR(arg_list); init_var;
						init_var = CDR(init_var))
	...
	case SYS_FUNC_NINTH:
		ret = CAR(
				CDR(CDR(CDR(CDR(CDR(CDR(CDR(CDR(CAR(arg_list)->Eval())))))))));
		break;
	case SYS_FUNC_TENTH:
		ret =
				CAR(
...
LObject *LSymbol::EvalUserFunction(LList *arg_list) {
	...
	// open block so that local vars aren't saved on the stack
	{
		...
		for (f_arg = fun_arg_list; f_arg; f_arg = CDR(f_arg))
			((LSymbol *) CAR(f_arg))->SetValue(
					(LObject *) l_user_stack.sdata[i++]);
...
void lisp_init() {
	...
	for (size_t i = 0; i < sizeof(sys_funcs) / sizeof(*sys_funcs); i++)
		add_sys_function(sys_funcs[i].name, sys_funcs[i].min_args,
				sys_funcs[i].max_args, i);
	clisp_init();
	current_space = TMP_SPACE;
