...
ico_switch_button::ico_switch_button(int X, int Y, int ID, int start_on,
		ifield *butts, ifield *Next) {
	x = X;
	y = Y;
	id = ID;
	next = Next;
...
void ico_switch_button::handle_event(event &ev, image *screen,
		InputManager *im) {
...
void ico_button::draw(int active, image *screen) {
	...
	if (up && !active)
		...
		cache.img(da)->put_image(screen, x1, y1);
...
void ico_button::deactivate_me()
{
	is_act = 0;
}

ico_button::ico_button(int X, int Y, int ID, int Up, int down, int upa,
		int downa, ifield *Next, int act_id, char const *help_key) {
	if (help_key) {
