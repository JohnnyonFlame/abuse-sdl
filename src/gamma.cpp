...
void gamma_correct(palette *&pal, int force_menu) {
	...
	if (gs && DEFINEDP(gs->GetValue()) && !force_menu) {
		...
		int wm_bc = wm->bright_color(), wm_mc = wm->medium_color(), wm_dc =
				wm->dark_color();
		...
		/*//TODO:: Code a proper gamma menu
		return;*/

		int sh = wm->font()->height() + 35;
		button *but = new button(5, 5 + sh * 3, ID_GAMMA_OK,
				cache.img(ok_button),
				new info_field(35, 10 + sh * 3, ID_NULL,
						lang_string("gamma_msg"), 0));

		gray_picker *gp = new gray_picker(2, 5 + sh, ID_GREEN_PICKER, 0, dg / 4,
				but);
		gp->set_pos(dg / 4);
		...
		do {
			...
			if (ev.type == EV_CLOSE_WINDOW)
				...
				gw->inm->handle_event(ev, gw);
		} while (!abort
				&& (ev.type != EV_MESSAGE || ev.message.id != ID_GAMMA_OK));
		...
		if (!abort) {
			...
			gammapath = (char *) malloc(
					strlen(get_save_filename_prefix()) + 10);
...
static void gamma_setgamma(palette *&pal, long gamma_v) {
	int abort = 0;
	...
	double gamma = log(gamma_v / 255.0) / log(16.0 / 255.0);
...
void gamma_correct(palette *&pal, int force_menu) {
	...
	if (!force_menu) {
		gamma_setgamma(pal, gamma);
	} else {
		int abort = 0;

		int fw = wm->font()->width();
		info_field *field = new info_field(0, 0, ID_NULL,
								" Press LEFT or RIGHT to change gamma.\n Press A to accept, B to revert.", 0);

		Jwindow *gw = wm->new_window((xres / 2) - (33 * fw / 2) - 35, yres / 2 - 90, -1, -1, field);
		event ev;
		...
		do {
			...
			if ((ev.type == EV_KEY && ev.key == JK_ALT_L) || (ev.type == EV_CLOSE_WINDOW)) {
				gamma_setgamma(pal, get_gamma());
				abort = 1;
			}
			...
			else if (ev.type == EV_KEY && ev.key == JK_LEFT) {
				gamma = Max(1, gamma - 1);
				gamma_setgamma(pal, gamma);
			}
			else if (ev.type == EV_KEY && ev.key == JK_RIGHT) {
				gamma = Min(128, gamma + 1);
				gamma_setgamma(pal, gamma);
			}
