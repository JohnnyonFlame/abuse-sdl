...
LSymbol *l_difficulty, *l_easy, *l_hard, *l_medium, *l_extreme, *l_max_hp,
		*l_max_power, *l_empty_cache;

// FIXME: port these to LSymbol
void *l_main_menu, *l_logo, *l_state_art, *l_abilities, *l_state_sfx,
		*l_song_list, *l_filename, *l_sfx_directory, *l_default_font, *l_morph,
		*l_default_abilities, *l_default_ai_function, *l_tile_files, *l_range,
		*l_hurt_all, *l_death_handler, *l_title_screen, *l_console_font,
		*l_fields, *l_dist, *l_pushx, *l_pushy, *l_object, *l_tile,
		*l_fire_object, *l_FIRE, *l_cop_dead_parts, *l_restart_player,
		*l_help_screens, *l_player_draw, *l_sneaky_draw, *l_health_image,
		*l_fly_image, *l_sneaky_image, *l_draw_fast, *l_player_tints,
		*l_save_order, *l_next_song, *l_level_load_start, *l_level_load_end,
		*l_cdc_logo, *l_keep_backup, *l_switch_to_powerful, *l_mouse_can_switch,
...
void *l_statbar_ammo_x, *l_statbar_ammo_y, *l_statbar_ammo_w, *l_statbar_ammo_h,
		*l_statbar_ammo_bg_color,

		*l_statbar_health_x, *l_statbar_health_y, *l_statbar_health_w,
		*l_statbar_health_h, *l_statbar_health_bg_color,
...
extern void ascatter_line(int x1, int y1, int x2, int y2, int c1, int c2,
		int s);
...
// to irnterface with c
{
	...
	l_statbar_health_bg_color = LSymbol::FindOrCreate(
...
void *l_caller(long number, void *args) {
	...
	case 4: {
		if (player_list->next)
			...
			return LPointer::Create(player_list->focus);
	}
		break;
	case 5:
		return LPointer::Create(
				current_level->find_closest(current_object->x,
						current_object->y, lnumber_value(CAR(args)->Eval()),
						current_object));
		break;
	case 6:
		return LPointer::Create(
				current_level->find_xclosest(current_object->x,
						current_object->y, lnumber_value(CAR(args)->Eval()),
						current_object));
		break;
	case 7: {
		...
		return LPointer::Create(
				current_level->find_xrange(current_object->x, current_object->y,
						n1, n2));
	}
		break;
	case 8: {
	...
	case 9:
		return LPointer::Create(the_game->first_view->focus);
		break;
	case 10: {
		view *v =
				((game_object *) lpointer_value(CAR(args)->Eval()))->controller()->next;
		if (v)
	...
	case 11: {
		return LPointer::Create(
				(void *) current_object->get_object(
						lnumber_value(CAR(args)->Eval())));
	}
		break;
	case 12: {
		return LPointer::Create(
				(void *) current_object->get_light(
						lnumber_value(CAR(args)->Eval())));
	}
		break;
	case 13: {
...
//      return current_lev shit
	...
	case 20: {
		...
		game_object *find = current_level->find_object_in_area(
				current_object->x, current_object->y, x1, y1, x2, y2, list,
				current_object);
		if (find)
	...
	case 21: {
		...
		void *list = CAR(args)->Eval();
		PtrRef r1(list);
		game_object *find = current_level->find_object_in_angle(
				current_object->x, current_object->y, a1, a2, list,
				current_object);
		if (find)
			return LPointer::Create(find);
		else
	...
	case 23: // def_character
	{
		...
		current_space = sp;
		if (!total_objects) {
			object_names = (char **) malloc(
					sizeof(char *) * (total_objects + 1));
			figures = (character_type **) malloc(
					sizeof(character_type *) * (total_objects + 1));
		} else {
			object_names = (char **) realloc(object_names,
					sizeof(char *) * (total_objects + 1));
			figures = (character_type **) realloc(figures,
					sizeof(character_type *) * (total_objects + 1));
		}
...
//      get_savegame_name(nm);
	...
	case 48: {
		...
		break;
	case 49: {
		int32_t x = lnumber_value(CAR(args)->Eval());
		args = CDR(args);
		int32_t y = lnumber_value(CAR(args)->Eval());
		args = CDR(args);
	...
	case 58: {
		...
		get_directory(lstring_value(CAR(args)->Eval()), files, tfiles, dirs,
				tdirs);
...
long c_caller(long number, void *args) {
	...
	case 1: {
		return abs(
				current_object->x - current_level->attacker(current_object)->x);
	}
		break;
	case 2: {
		return abs(
				current_object->y - current_level->attacker(current_object)->y);
	}
		break;
	case 3: {
		if (!current_object->controller()) {
			...
			return current_object->controller()->key_down(
					lnumber_value(CAR(args)));
		}
	}
		break;
	case 4: {
	...
	case 12: {
		...
		break;
	case 13: {
		return current_object->move(lnumber_value(CAR(args)),
...
//      current_object->last_y=v;
	...
	case 34: {
		return current_level->push_characters(current_object,
				lnumber_value(CAR(args)), lnumber_value(CAR(CDR(args))));
	}
		break;
	...
	case 50:
		...
		break;
	case 51:
		write_PCX(screen, pal, lstring_value(CAR(args)));
		break;
	...
	case 73:
		the_game->set_level(
				new level(lnumber_value(CAR(args)),
						lnumber_value(CAR(CDR(args))),
	...
	case 75: {
		...
		o->do_damage(amount, current_object, current_object->x,
				current_object->y, xv, yv);
	}
		break;
	case 76:
	...
	case 77: {
		...
		if (!o->controller())
			...
			o->controller()->shift_down = lnumber_value(CAR(CDR(args)));
		return 1;
	}
		break;
	case 78: {
		...
		if (!o->controller())
			...
			o->controller()->shift_right = lnumber_value(CAR(CDR(args)));
		return 1;
	}
		break;
	case 79:
	...
	case 91:
		...
		break;
	case 92: {
		int32_t cx1, x1 = lnumber_value(CAR(args));
		args = lcdr(args);
		int32_t cy1, y1 = lnumber_value(CAR(args));
		args = lcdr(args);
		int32_t cx2, x2 = lnumber_value(CAR(args));
		args = lcdr(args);
		int32_t cy2, y2 = lnumber_value(CAR(args));
		args = lcdr(args);
	...
	case 99:
		current_object->remove_object(
				(game_object *) lpointer_value(CAR(args)));
		return 1;
		break;
	case 100:
		...
		break;
	case 101:
		current_object->remove_light(
				(light_source *) lpointer_value(CAR(args)));
		return 1;
		break;
	case 102:
	...
	case 128: {
		return current_level->platform_push(current_object,
				lnumber_value(CAR(args)), lnumber_value(CAR(CDR(args))));
	}
		break;
	case 133: // def_sound
	{
		...
		if (CDR(args)) {
			...
			if (item_type(sym) != L_SYMBOL) {
				lbreak(
						"expecting first arg to def-character to be a symbol!\n");
				exit(0);
			}
			args = CDR(args);
		}
	...
	case 151: {
		...
		if (a < 0 || a >= TOTAL_ABILITIES) {
			((LObject *) args)->Print();
			lbreak(
					"bad ability number for get_ability, should be 0..%d, not %d\n",
					TOTAL_ABILITIES, a);
			exit(0);
		}
	...
	case 153: {
		game_object *o = (game_object *) lpointer_value(CAR(args));
		int32_t x = o->x - current_object->x, y =
				-(o->y - o->picture()->Size().y / 2
						- (current_object->y
								- (current_object->picture()->Size().y / 2)));
...
//      stat_bar=cache.reg_object(fn,CAR(args),SPEC_IMAGE,1);
	...
	case 192: {
		...
		if (x < 0 || y < 0 || x >= current_level->foreground_width()
				|| y >= current_level->foreground_width())
	...
	case 197: {
		...
		if (fp->open_failure()) {
			delete fp;
			lbreak("load_palette : could not open file %s for reading",
	...
	case 198: {
		...
		if (fp->open_failure()) {
			delete fp;
			lbreak("load_color_filter : could not open file %s for reading",
					lstring_value(CAR(args)));
			exit(1);
		} else {
			spec_directory sd(fp);
			...
			if (!se)
				lbreak("File %s has no color filter!",
						lstring_value(CAR(args)));
			else {
				delete color_table;
	...
	case 200: {
		...
		current_object->try_move(current_object->x, current_object->y, xv, yv,
				1 | top);
		current_object->x += xv;
		current_object->y += yv;
	...
	case 208: {
		current_object->draw_trans(lnumber_value(CAR(args)),
				lnumber_value(CAR(CDR(args))));
	}
		break;
	case 209: {
	...
	case 210: {
		...
		break;
	case 211: {
		return lget_view(CAR(args), "shift_down")->shift_right;
	}
		break;
	case 212: {
		return lget_view(CAR(args), "shift_right")->shift_down;
	}
		break;
	case 213: {
	...
	case 221: {
		...
		if (!v)
			...
			v->last_y -= lnumber_value(CAR(args));
	}
		break;
	case 222: {
		view *v = current_object->controller();
	...
	case 225: {
		...
		if (strncmp(lstring_value(CAR(args)), "save", 4) == 0) {
			sprintf(fn, "%s%s", get_save_filename_prefix(),
					lstring_value(CAR(args)));
		} else {
	...
	case 266: {
		...
		break;
	case 267: {
		current_object->draw_double_tint(lnumber_value(CAR(args)),
				lnumber_value(CAR(CDR(args))));
	}
		break;
	case 268: {
