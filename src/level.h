...
public:
	...
	area_controller(int32_t X, int32_t Y, int32_t W, int32_t H,
			area_controller *Next);
};
...
public:
	...
	uint16_t get_fg(int x, int y) {
		if (x >= 0 && y >= 0 && x < fg_width && y < fg_height)
			return fgvalue(*(map_fg+x+y*fg_width));
		else
			return 0;
	}
	uint16_t get_bg(int x, int y) {
		if (x >= 0 && y >= 0 && x < bg_width && y < bg_height)
			return *(map_bg + x + y * bg_width);
		else
			return 0;
	}
	void put_fg(int x, int y, uint16_t tile) {
		*(map_fg + x + y * fg_width) = tile;
	}
	void put_bg(int x, int y, uint16_t tile) {
		*(map_bg + x + y * bg_width) = tile;
	}
	...
	game_object *damage_intersect(int32_t x1, int32_t y1, int32_t &x2,
			int32_t &y2, game_object *exclude);
	game_object *boundary_setback(game_object *subject, int32_t x1, int32_t y1,
			int32_t &x2, int32_t &y2);
	game_object *all_boundary_setback(game_object *subject, int32_t x1,
	...
	void hurt_radius(int32_t x, int32_t y, int32_t r, int32_t m,
	...
	void write_links(bFILE *fp, object_node *save_list,
			object_node *exclude_list);
	void load_links(bFILE *fp, spec_directory *sd, object_node *save_list,
			object_node *exclude_list);
...
//  game_object *find_enemy(game_object *exclude1, game_object *exclude2);
	...
	bFILE *create_dir(char *filename, int save_all, object_node *save_list,
			object_node *exclude_list);
	...
	game_object *find_object_in_area(int32_t x, int32_t y, int32_t x1,
			int32_t y1, int32_t x2, int32_t y2, Cell *list,
			game_object *exclude);
	game_object *find_object_in_angle(int32_t x, int32_t y, int32_t start_angle,
			int32_t end_angle, void *list, game_object *exclude);
