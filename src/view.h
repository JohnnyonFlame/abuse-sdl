...
public:
	...
	int
			x_suggestion, // input from the player at the current time
			y_suggestion, b1_suggestion, b2_suggestion, b3_suggestion,
			b4_suggestion, pointer_x, pointer_y, freeze_time;

	short ambient; // ambient lighting setting, used by draw

	int32_t pan_x, pan_y, no_xleft, no_xright, no_ytop, no_ybottom, last_x,
			last_y, last_last_x, last_last_y, view_percent;
	...
	int has_weapon(int weapon_type) {
		return god || (weapons[weapon_type] != -1);
	}
	...
	void set_input(int cx, int cy, int b1, int b2, int b3, int b4, int px,
			int py);
	int view_changed() {
