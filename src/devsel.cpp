...
tile_picker::tile_picker(int X, int Y, int ID, int spec_type, int Scale,
		int scroll_h, int Wid, ifield *Next) :
...
void tile_picker::scroll_event(int newx, image *screen) {
	...
	for (int i = newx; i < newx + th * wid; i++) {
		...
		if (i < t) {
			...
			case SPEC_FORETILE: {
				if (foretiles[i] < 0)
					...
					if (rev) {
						screen->bar(xo, yo, xo + xw - 1, yo + ya - 1,
								wm->bright_color());
			...
			case SPEC_BACKTILE: {
				...
				break;
			case SPEC_CHARACTER: {
				figures[i]->get_sequence(stopped)->get_figure(0)->forward->PutImage(
						&im, vec2i(0, 0));
				scale_put(&im, screen, xo, yo, xw, ya);
			}
				break;
		...
		if (i == c)
			screen->rectangle(xo, yo, xo + xw - 1, yo + ya - 1,
					wm->bright_color());

	}
}

void tile_picker::handle_inside_event(event &ev, image *screen,
		InputManager *inm) {
	if (ev.type == EV_MOUSE_BUTTON) {
		int sel = ((ev.mouse_move.y - y) / pich() * wid)
				+ (ev.mouse_move.x - x) / picw() + last_sel;
