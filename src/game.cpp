...
void Game::grow_views(int amount) {
	...
	for (f = first_view; f; f = f->next) {
		...
		if (f->suggest.cx2 - f->suggest.cx1 < 20
				|| f->suggest.cy2 - f->suggest.cy1 < 15 || f->suggest.cx1 < 0
				|| f->suggest.cy1 < 0)
...
//int prev_xoff=0, prev_yoff=0;
void Game::mouse_to_game(int32_t x, int32_t y, int32_t &gamex, int32_t &gamey,
		view *f) {
	if (!f)
		f = view_in(x, y);
	if (!f)
	...
	if (f) {
		if (dev & MAP_MODE) {
			gamex = (x - (int32_t) f->cx1) * ftile_width() / AUTOTILE_WIDTH
					+ map_xoff * ftile_width();
			gamey = (y - (int32_t) f->cy1) * ftile_height() / AUTOTILE_HEIGHT
					+ map_yoff * ftile_height();
		} else {
	...
	/*gamex = (f->xoff() - prev_xoff) * 2048;
	gamey = (f->yoff() - prev_yoff) * 2048;
...
void Game::game_to_mouse(int32_t gamex, int32_t gamey, view *which, int32_t &x,
	...
	x = gamex * AUTOTILE_WIDTH / ftile_width() - x1 * AUTOTILE_WIDTH
			+ which->cx1;
	if (x1 > 0)
		x -= (which->focus->x * AUTOTILE_WIDTH / ftile_width()) % AUTOTILE_WIDTH;

	y = gamey * AUTOTILE_HEIGHT / ftile_height() - y1 * AUTOTILE_HEIGHT
			+ which->cy1;
	if (y1 > 0)
...
void Game::set_state(int new_state) {
	...
	if (playing_state(new_state) && !playing_state(state)) {
		...
		d = 1;
	} else if (!playing_state(new_state)
...
void Game::joy_calb(event &ev) {
	...
	if (ev.type == EV_SPURIOUS) // spurious means we should update our status
	{
		...
		joy_win->screen->bar(dx, dy, dx + jim->Size().x + 6,
				dy + jim->Size().y + 6, wm->black());
...
void Game::draw_value(image *screen, int x, int y, int w, int h, int val,
		int max) {
...
void Game::put_block_fg(int x, int y, TransImage *im) {
	for (view *f = first_view; f; f = f->next) {
		if (f->drawable()) {
			int xoff = f->xoff(), yoff = f->yoff(), viewx1 = f->cx1, viewy1 =
					f->cy1, viewx2 = f->cx2, viewy2 = f->cy2;
			if (xoff / ftile_width() > x
					|| xoff / ftile_width() + (viewx2 - viewx1) / ftile_width()
							+ 1 < x || yoff / ftile_height() > y
					|| yoff / ftile_height()
							+ (viewy2 - viewy1) / ftile_height() + 1 < y)
				return;
			int cx1, cy1, cx2, cy2;
			...
			im->PutImage(screen,
					vec2i(
							(x - xoff / ftile_width()) * ftile_width() + viewx1
									- xoff % ftile_width(),
							(y - yoff / ftile_height()) * ftile_height()
									+ viewy1 - yoff % ftile_height()));
...
void Game::put_block_bg(int x, int y, image *im) {
	for (view *f = first_view; f; f = f->next) {
		if (f->drawable()) {
			...
			if (xo / btile_width() > x
					|| xo / btile_width() + (viewx2 - viewx1) / btile_width()
							+ 1 < x || yo / btile_height() > y
					|| yo / btile_height() + (viewy2 - viewy1) / btile_height()
							+ 1 < y)
				return;
			int cx1, cy1, cx2, cy2;
			screen->GetClip(cx1, cy1, cx2, cy2);
			screen->SetClip(viewx1, viewy1, viewx2 + 1, viewy2 + 1);
			im->put_image(screen,
					(x - xo / btile_width()) * btile_width() + viewx1
							- xo % btile_width(),
					(y - yo / btile_height()) * btile_height() + viewy1
							- yo % btile_height(), 0);
			screen->SetClip(cx1, cy1, cx2, cy2);
		}
	}
...
void Game::dev_scroll() {
	...
	if (dev) {
		...
		if (mousex < xmargin && dev_cont->ok_to_scroll())
			xs = -18;
		else if (mousex > (screen->Size().x - xmargin)
				&& dev_cont->ok_to_scroll())
			xs = 18;
		else if (wm->key_pressed(JK_LEFT) && !last_input
				&& !dev_cont->need_arrows())
			xs = -18;
		else if (wm->key_pressed(JK_RIGHT) && !last_input
				&& !dev_cont->need_arrows())
			xs = 18;
		else
			xs = 0;
		...
		if (mousey < ymargin && dev_cont->ok_to_scroll())
			ys = -18;
		else if (mousey > (screen->Size().y - ymargin)
				&& dev_cont->ok_to_scroll())
			ys = 18;
...
void Game::draw_map(view *v, int interpolate) {
	...
	if (small_render)
		screen->AddDirty(v->cx1, v->cy1, (v->cx2 - v->cx1 + 1) * 2 + v->cx1 + 1,
				v->cy1 + (v->cy2 - v->cy1 + 1) * 2 + 1);
	else
...
//  if(yoff > max_yoff) yoff = max_yoff;
	...
	if (!(dev & MAP_MODE) && (dev & DRAW_BG_LAYER)) {
		...
		int bh = current_level->background_height(), bw =
		...
		if (dev & MAP_MODE) {
			...
			for (y = y1, draw_y = yo; y <= y2; y++, draw_y += yinc) {
				if (!(draw_y < ncy1 || draw_y + yinc > ncy2)) {
					...
					for (x = x1, draw_x = xo; x <= x2;
							x++, cl++, sl1 += xinc, draw_x += xinc) {
...
//          else fort_num = 0;
							...
							uint8_t *sl2 =
									get_fg(fort_num)->micro_image->scan_line(0);
							uint8_t *sl3 = sl1;
			...
			int fg_h = current_level->foreground_height(), fg_w =
				...
				for (x = x1, draw_x = xo; x <= x2; x++, draw_x += xinc, cl++) {
					if (x < fg_w && y < fg_h) {
						if (above_tile(*cl))
							...
							if (fort_num != BLACK) {
								get_fg(fort_num)->im->PutImage(screen,
										vec2i(draw_x, draw_y));
					...
					if (above_tile(*cl)) {
						...
						if (fort_num != BLACK) {
							if (dev & DRAW_BG_LAYER)
								...
								get_fg(fort_num)->im->PutFilled(screen,
										vec2i(draw_x, draw_y), 0);

							if (!(dev & EDIT_MODE))
								current_level->mark_seen(x, y);
							else {
								screen->line(draw_x, draw_y, draw_x + xinc,
										draw_y + yinc, wm->bright_color());
								screen->line(draw_x + xinc, draw_y, draw_x,
										draw_y + yinc, wm->bright_color());
							}
						}
					}
						...
						if (fort_num != BLACK) {
							...
							if (p->tot) {
								for (int i = 1; i < p->tot; i++) {
									d += 2;
									screen->line(draw_x + *(d - 2),
											draw_y + *(d - 1), draw_x + *d,
											draw_y + *(d + 1), b);
								}
								screen->line(draw_x + *d, draw_y + *(d - 1),
										draw_x + p->data[0],
										draw_y + p->data[1], b);
							}
						}
...
//      server_check();
		...
		if (dev & DRAW_HELP_LAYER) {
			if (help_text_frames >= 0) {
				...
				int x1 = v->cx1, y1 = v->cy1, x2 = v->cx2, y2 = v->cy1
						+ wm->font()->height() + 10;
				...
				wm->font()->put_string(screen, x1 + 5, y1 + 5, help_text,
						color);
				if (color > 30)
					help_text_frames = -1;
				else
		...
		if (dev_cont)
			dev_cont->dev_draw(v);
		if (cache.in_use())
			cache.img(vmm_image)->put_image(screen, v->cx1,
					v->cy2 - cache.img(vmm_image)->Size().y + 1);

		if (dev & DRAW_LIGHTS) {
			if (small_render) {
				double_light_screen(screen, xoff, yoff, white_light, v->ambient,
						old_screen, old_cx1, old_cy1);
...
int Game::in_area(event &ev, int x1, int y1, int x2, int y2) {
	return (last_demo_mx >= x1 && last_demo_mx <= x2 && last_demo_my >= y1
			&& last_demo_my <= y2);
}
...
int text_draw(int y, int x1, int y1, int x2, int y2, char const *buf,
		JCFont *font, uint8_t *cmap, char color);
...
void do_title() {
	...
	if (!fp->open_failure()) {
		...
		int dx = (xres + 1) / 2 - gray->Size().x / 2, dy = (yres + 1) / 2
				- gray->Size().y / 2;
		...
		uint8_t cmap[32];
		for (int i = 0; i < 32; i++)
			cmap[i] = pal->find_closest(i * 256 / 32, i * 256 / 32,
					i * 256 / 32);
		...
		while (ev.type != EV_KEY && ev.type != EV_MOUSE_BUTTON) {
			...
			text_draw(205 - i, dx + 15, dy, dx + 320 - 15, dy + 199, str,
					wm->font(), cmap, wm->bright_color());
			wm->flush_screen();
			time_marker now;
			...
			if ((i % 5) == 0 && DEFINEDP(space_snd)
					&& (sound_avail & SFX_INITIALIZED))
				cache.sfx(lnumber_value(space_snd))->play(
						sfx_volume * 90 / 127);
...
//    load_level(NET_STARTFILE);
	...
	morph_bright_color = get_color(
			cache.img(window_colors)->Pixel(vec2i(0, 1)));
	...
	wm = new WindowManager(screen, pal, bright_color, med_color, dark_color,
			game_font);
	...
	dev |= DRAW_FG_LAYER | DRAW_BG_LAYER | DRAW_PEOPLE_LAYER | DRAW_HELP_LAYER
			| DRAW_LIGHTS | DRAW_LINKS;
...
void Game::show_time() {
	...
	console_font->put_string(screen, first_view->cx1, first_view->cy1 + 10,
			str);
}
...
void Game::update_screen() {
	if (state == HELP_STATE)
		...
		if (!(dev & EDIT_MODE) || refresh) {
			...
			for (; f; f = f->next) {
				if (f->focus) {
					...
					total_active += current_level->add_drawables(
							f->xoff() - w / 4, f->yoff() - h / 4,
		...
		if (state == PAUSE_STATE) {
			for (view *f = first_view; f; f = f->next)
				cache.img(pause_image)->put_image(screen,
						(f->cx1 + f->cx2) / 2
								- cache.img(pause_image)->Size().x / 2,
						f->cy1 + 5, 1);
		}
...
void Game::get_input() {
	...
	while (event_waiting()) {
		...
		if (ev.type != EV_KEY || !key_down(ev.key) || ev.window
				|| (dev & EDIT_MODE)) {
#if !defined __CELLOS_LV2__
			if (ev.type == EV_KEY) {
			...
			case PAUSE_STATE: {
				if (ev.type == EV_KEY
						&& (ev.key == JK_SPACE || ev.key == JK_CTRL_L)) {
					set_state(RUN_STATE);
				}
			}
				break;
				...
				if (ev.window == NULL) {
					...
					case EV_KEY: {
						...
						case 'm': {
							if (dev & MAP_MODE)
								dev -= MAP_MODE;
							else if ((player_list && player_list->next)
									|| dev & EDIT_MODE)
								dev |= MAP_MODE;
						...
						case 'p': {
							if (!(dev & EDIT_MODE)
									&& (!main_net_cfg
											|| (main_net_cfg->state
													!= net_configuration::SERVER
													&& main_net_cfg->state
															!= net_configuration::CLIENT))) {
					...
					case EV_MESSAGE: {
						...
						case LOWER_MUSIC: {
							...
							if (ev.message.id == RAISE_MUSIC
									&& music_volume != 126) {
								music_volume = Min(music_volume + 16, 127);
								if (current_song
										&& (sound_avail & MUSIC_INITIALIZED))
									current_song->set_volume(music_volume);
							}

							if (ev.message.id == LOWER_MUSIC
									&& music_volume != 0) {
...
void Game::step() {
	...
	if (current_level) {
		...
		for (view *f = first_view; f; f = f->next) {
			if (f->focus) {
				...
				w = (f->cx2 - f->cx1 + 1);
				h = (f->cy2 - f->cy1 + 1);
				total_active += current_level->add_actives(f->xoff() - w / 4,
						f->yoff() - h / 4, f->xoff() + w + w / 4,
						f->yoff() + h + h / 4);
			}
		}
	}
	...
	if (state == RUN_STATE) {
		...
		if (demo_man.current_state() == demo_manager::NORMAL && idle_ticks > 420
				&& demo_start) {
			idle_ticks = 0;
			set_state(MENU_STATE);
		...
		{
			//start button
			if (key_down(JK_ENTER)) {
				set_state(MENU_STATE);
				set_key_down(JK_ENTER, 0);
			}
			ambient_ramp = 0;
			view *v;
	...
	if ((key_down('x') || key_down(JK_F4))
			&& (key_down(JK_ALT_L) || key_down(JK_ALT_R)) && confirm_quit())
		finished = true;
}
...
void Game::draw(int scene_mode) {
	...
	if (scene_mode) {
		...
		wm->font()->put_string(screen,
				screen->Size().x / 2
						- (wm->font()->width() * strlen(helpstr)) / 2 + 1,
				screen->Size().y - wm->font()->height() - 5 + 1, helpstr,
				wm->dark_color());
		wm->font()->put_string(screen,
				screen->Size().x / 2
						- (wm->font()->width() * strlen(helpstr)) / 2,
	...
	*st = 0;
	if (dev_console && !external_print) {
		...
		do {
			...
			wm->flush_screen();
		} while (ev.type != EV_KEY || ev.key != JK_CTRL_L);
		dprintf("\n");
	} else {
