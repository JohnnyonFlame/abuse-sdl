...
void get_event(event &ev) {
	...
	case EV_KEY: {
		...
		else if (ev.key == JK_ENTER
				&& demo_man.state == demo_manager::RECORDING) {
			demo_man.set_state(demo_manager::NORMAL);
...
void demo_manager::do_inputs() {
	...
	case RECORDING: {
		...
		demo_man.save_packet(base->packet.packet_data(),
				base->packet.packet_size());
		process_packet_commands(base->packet.packet_data(),
				base->packet.packet_size());
	...
	case PLAYING: {
		...
		if (get_packet(buf, size)) // get starting inputs
			...
			int32_t mx, my;
			the_game->game_to_mouse(player_list->pointer_x,
					player_list->pointer_y, player_list, mx, my);
			wm->set_mouse_position(small_render ? mx * 2 : mx,
					small_render ? my * 2 : my);
		} else {
			set_state(NORMAL);
			return;
		}
