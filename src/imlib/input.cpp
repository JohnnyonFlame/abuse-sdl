...
button_box::button_box(int X, int Y, int ID, int MaxDown, button *Buttons,
		ifield *Next) {
	x = X;
	y = Y;
	id = ID;
		...
		for (button *b = buttons; !found && b; b = (button *) b->next) // see if the user clicked on a button
			...
			if (ev.mouse_move.x >= x1 && ev.mouse_move.x <= x2
				...
				if (*((int *) b->read()) == 0) // did the user press or release the button
						{
					if (total > maxdown) {
						for (b2 = buttons; total > maxdown && b2; b2 =
								(button *) b2->next)
							if ((b != b2 || maxdown == 0)
									&& *((int *) b2->read()) == 0) {
								total--;
...
button::button(int X, int Y, int ID, image *Depressed, image *Pressed,
		image *active, ifield *Next) {
	x = X;
	y = Y;
	id = ID;
	text = NULL;
...
void button::handle_event(event &ev, image *screen, InputManager *im) {
	if ((ev.type == EV_KEY && ev.key == JK_CTRL_L)
...
void button::draw(int active, image *screen) {
	int x1, y1, x2, y2, color = (
			active ? wm->bright_color() : wm->medium_color());
	area(x1, y1, x2, y2);
...
//      screen->widget_bar(,wm->bright_color(),wm->medium_color(),wm->dark_color());
			screen->widget_bar(x1 + 1, y1 + 1, x2 - 1, y2 - 1,
			...
			if (text) {
				...
				visual->put_image(screen, x + 3, y + 3, 1);
		} else {
...
void text_field::draw_cur(int color, image *screen) {
	screen->bar(xstart() + cur * wm->font()->width() + 1, yend() - 2,
			xstart() + (cur + 1) * wm->font()->width(), yend() - 1, color);
}
...
void info_field::draw_first(image *screen) {
	put_para(screen, text, x + 1, y + 1, wm->font()->width(),
			wm->font()->height(), wm->font(), wm->black());
	put_para(screen, text, x, y, wm->font()->width(), wm->font()->height(),
			wm->font(), wm->bright_color());
}
