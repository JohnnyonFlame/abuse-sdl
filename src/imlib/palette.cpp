...
int palette::find_closest_non0(unsigned char r, unsigned char g,
		unsigned char b) {
...
void palette::black_white() {
	...
	for (i = 0; i < 256; i++) {
		get(i, r, g, b);
		gr = (unsigned char) ((double) r * 0.30 + (double) g * 0.59
				+ (double) b * 0.11);
		set(i, gr, gr, gr);
	}
}
...
int palette::add_color(unsigned int r, int unsigned g, int unsigned b,
		int closest_only) {
	int i, f, u, c;
	if (!closest_only) {
...
void palette::set(int x, unsigned char red, char unsigned green,
		char unsigned blue) {
...
void palette::get(int x, unsigned char &red, unsigned char &green,
		unsigned char &blue) {
...
palette::palette(int number_colors) {
	CONDITION(number_colors>0,
			"palette::constructor - need at least one color!");
...
quant_node::quant_node(int level, quant_node *dad, unsigned char r,
...
void quant_palette::add_color(unsigned char r, unsigned char g,
		unsigned char b) {
	quant_node **p, *fat;
	int lev, cn, stop;
	p = &root;
...
palette *quant_palette::create_pal() {
	...
	for (x = 0, i = 7; i >= 0; i++)
		for (pn = (quant_node *) level[i].first();
				pn != (quant_node *) level[i].first();
				pn = (quant_node *) pn->Next())
			if (pn->is_leaf())
...
void palette::fade_to(int total_fades, int fade_on, int dest_r, int dest_g,
		int dest_b) {
