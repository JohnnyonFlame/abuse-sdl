...
void WindowManager::hide_window(Jwindow *j) {
	if (j == first)
		...
		for (k = first; k->next != j; k = k->next)
			k->screen->AddDirty(j->x - k->x, j->y - k->y, j->x + j->l - k->x,
					j->y + j->h - k->y);
		k->screen->AddDirty(j->x - k->x, j->y - k->y, j->x + j->l - k->x,
				j->y + j->h - k->y);
		k->next = j->next;
...
void WindowManager::get_event(event &ev) {
	...
	if (state == inputing) {
		for (ev.window = NULL,j = first; j; j = j->next)
			if (!j->is_hidden() && ev.mouse_move.x >= j->x
					&& ev.mouse_move.y >= j->y && ev.mouse_move.x < j->x + j->l
					&& ev.mouse_move.y < j->y + j->h)
				ev.window = j;
		...
		if (ev.window) {
			...
			if ((ev.type == EV_MOUSE_BUTTON && ev.mouse_button == 1 && ev.window
					&& ev.mouse_move.x >= ev.window->x
					&& ev.mouse_move.y >= ev.window->y
					&& ev.mouse_move.x < ev.window->x + ev.window->l
					&& ev.mouse_move.y < ev.window->y + ev.window->y1())) {
				if (ev.mouse_move.x - ev.window->x < 11)
					closew = 1;
		...
		if (ev.type == EV_MOUSE_BUTTON && ev.mouse_button == 0) // user released the mouse
			...
			move_window(drag_window, ev.mouse_move.x + drag_mousex,
	...
	if (ev.type == EV_REDRAW) {
		...
		screen->AddDirty(ev.redraw.x1, ev.redraw.y1, ev.redraw.x2 + 1,
				ev.redraw.y2 + 1);
		flush_screen();
...
void WindowManager::resize_window(Jwindow *j, int l, int h) {
	...
	screen->AddDirty(j->x, j->y, j->x + j->l, j->y + j->h);
	for (p = first; p != j; p = p->next)
		p->screen->AddDirty(j->x - p->x, j->y - p->y, j->x + j->l - p->x,
				j->y + j->h - p->y);
	j->resize(l, h);
	if (!frame_suppress)
		j->redraw();
...
WindowManager::WindowManager(image *Screen, palette *Pal, int Hi, int Med,
		int Low, JCFont *Font) {
	wm = this;
	screen = Screen;
	hi = Hi;
	...
	if (first == win)
		...
		for (search = first; search->next != win; search = search->next)
			search->screen->AddDirty(win->x - search->x, win->y - search->y,
					win->x + win->l - search->x, win->y + win->h - search->y);
		search->screen->AddDirty(win->x - search->x, win->y - search->y,
				win->x + win->l - search->x, win->y + win->h - search->y);
...
Jwindow * WindowManager::new_window(int x, int y, int l, int h, ifield * fields,
		char const *name) {
...
void WindowManager::flush_screen() {
	...
	if (has_mouse()) {
		...
		screen->put_part(mouse_save, 0, 0, mx, my, mx + mouse_pic->Size().x - 1,
...
void Jwindow::redraw() {
	...
	if (_name) {
		if (right_border() >= 1) {
			...
			if (right_border() >= 3)
				screen->widget_bar(right_border() - 1, top_border() - 1,
						l - left_border(), h - bottom_border(), low, med, hi);
		...
		screen->rectangle(2, 2, top_border() - 2, top_border() - 3,
				wm->black());
		screen->widget_bar(3, 3, top_border() - 3, top_border() - 4, hi, med,
...
void InputManager::handle_event(event &ev, Jwindow *j) {
	...
	if (!grab) {
		if ((ev.type == EV_MOUSE_BUTTON && ev.mouse_button == 1)
			...
			if (in_area != active
					&& (no_selections_allowed
							|| (in_area && in_area->selectable()))) {
				if (active)
					active->draw(0, screen);
		...
		if (ev.type == EV_KEY && (ev.key == JK_UP))
		{
			if (active) {
				active->draw(0, screen);
			}
			...
			if (active && (active != first)) {
				for(iterator = first, old = active; (iterator != old) && iterator; iterator = iterator->next)
				{
			...
			{
				for (iterator = first, old = active; iterator; iterator = iterator->next)
				{
					if (iterator->selectable())
						active = iterator;
				}
	...
	if (active) {
		if (ev.type == EV_KEY && ((ev.key == JK_LEFT) || (ev.key == JK_RIGHT)))
		{
			if (j)
		...
		else if (ev.type != EV_MOUSE_MOVE && ev.type != EV_MOUSE_BUTTON)
			active->handle_event(ev, screen, this);
		else {
			active->area(x1, y1, x2, y2);
			if (grab
					|| (ev.mouse_move.x >= x1 && ev.mouse_move.y >= y1
							&& ev.mouse_move.x <= x2 && ev.mouse_move.y <= y2)) {
				if (j)
					active->handle_event(ev, screen, j->inm);
				else
