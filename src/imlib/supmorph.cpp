...
super_morph::super_morph(TransImage *hint1, TransImage *hint2, int aneal_steps,
		void (*stat_fun)(int)) {
	int x, y, w1 = hint1->Size().x, h1 = hint1->Size().y, w2 = hint2->Size().x,
			h2 = hint2->Size().y;
	if (w1 > w2)
		w = w1;
	else
	...
	unsigned short start1[256], start2[256], totals[256], start = 0, *h1p =
			hints1, *h2p = hints2;
	...
	for (start = 0, x = 0; x < total_hints; x++) {
		y = hint_color[x];
		int dups;
		for (dp = movers + start1[y] * 4, dups = totals[y] - hints1[y]; dups;
				dups--) {
			*dp = *(dp - 4);
	...
	for (start = 0, x = 0; x < total_hints; x++) {
		y = hint_color[x];
		int dups;
		for (dp = movers + start2[y] * 4 + 2, dups = totals[y] - hints2[y];
				dups; dups--) {
			*dp = *(dp - 4);
			dp++; // copy previous x,y position
			*dp = *(dp - 4);
	...
	for (y = 0; y < aneal_steps; y++) {
		...
		for (x = 0; x < total_hints; x++) {
			...
			for (a = 0; a < z; a++, dp += 4) {
				unsigned char *swap = range_start
						+ (rtable[((rand_on++) & (RAND_TABLE_SIZE - 1))] % z)
								* 4;
				int d_old = p_dist(dp[0],dp[1],dp[2],dp[3])
						+ p_dist(swap[0],swap[1],swap[2],swap[3]);
				int d_new = p_dist(dp[0],dp[1],swap[2],swap[3])
						+ p_dist(swap[0],swap[1],dp[2],dp[3]);
				if (d_new < d_old) {
					unsigned char s;
					s = swap[2];
					swap[2] = dp[2];
...
smorph_player::smorph_player(super_morph *m, palette *pal, image *i1, image *i2,
		int frames, int dir) {
	int i, x1, y1, x2, y2;
...
int smorph_player::show(image *screen, int x, int y, ColorFilter *fil,
		palette *pal, int blur_threshold) {
	if (f_left) {
		int i, px, py, ix, iy;
		int x1, y1, x2, y2;
		...
		for (ss = steps, i = 0; i < t; i++, ss++) {
			...
			if (px >= x1 && px < x2 && py >= y1 && py < y2) {
				hole[ix + iy * w] = *(screen->scan_line(py) + px) = fil->Lookup(
						ss->r >> (19), ss->g >> (19), ss->b >> (19));
			}
			ss->x += ss->dx;
			ss->y += ss->dy;
			ss->r += ss->dr;
