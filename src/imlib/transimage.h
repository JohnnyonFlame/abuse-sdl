...
private:
	uint8_t *ClipToLine(image *screen, vec2i pos1, vec2i pos2, vec2i &posy,
			int &ysteps);
	...
	template<int N>
	void PutImageGeneric(image *dest, vec2i pos, uint8_t color, image *blend,
			vec2i bpos, uint8_t *map1, uint8_t *map2, int amount, int nframes,
			uint8_t *tint, ColorFilter *f, palette *pal);
