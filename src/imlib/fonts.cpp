...
void texture_font::put_char(image *screen, int x, int y, char ch) {
	if (fntpat)
		fntpat->put_part_masked(screen, let, x, y, ((int) ch % 32) * tl,
				((int) ch / 32) * th, 0, 0, tl - 1, th - 1);
	else
		let->put_part(screen, x, y, ((int) ch % 32) * tl, ((int) ch / 32) * th,
				((int) ch % 32) * tl + tl - 1, ((int) ch / 32) * th + th - 1,
				1);
}
...
void JCFont::put_string(image *screen, int x, int y, char const *st,
		int color) {
	while (*st) {
...
void JCFont::put_char(image *screen, int x, int y, char ch, int color) {
	if (let[(int) ch]) {
		if (color >= 0)
			...
			let[(int) ch]->PutImage(screen, vec2i(x, y));
	}
}
...
JCFont::JCFont(image *letters) {
	...
	for (ch = 0; ch < 256; ch++) {
		tmp.clear();
		letters->put_part(&tmp, 0, 0, ((int) ch % 32) * tl,
				((int) ch / 32) * th, ((int) ch % 32) * tl + tl - 1,
				((int) ch / 32) * th + th - 1, 1);
