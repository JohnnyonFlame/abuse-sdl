...
uint8_t *TransImage::ClipToLine(image *screen, vec2i pos1, vec2i pos2,
	...
	if (pos.y + m_size.y <= pos1.y || pos.y >= pos2.y || pos.x >= pos2.x
			|| pos.x + m_size.x <= pos1.x)
		return NULL;
	...
	screen->AddDirty(Max(pos.x, pos1.x), pos.y, Min(pos.x + m_size.x, pos2.x),
			pos.y + m_size.y);
	return parser;
}
...
void TransImage::PutImageGeneric(image *screen, vec2i pos, uint8_t color,
		image *blend, vec2i bpos, uint8_t *map, uint8_t *map2, int amount,
		int nframes, uint8_t *tint, ColorFilter *f, palette *pal) {
	vec2i pos1, pos2;
	int ysteps, mul = 0;
	...
	uint8_t *datap = ClipToLine(screen, pos1, pos2, pos, ysteps), *screen_line,
			*blend_line = NULL, *paddr = NULL;
	if (!datap)
		return; // if ClipToLine says nothing to draw, return

	CONDITION(
			N == BLEND && pos.y >= bpos.y && pos.y + ysteps < bpos.y + blend->Size().y + 1,
	...
	for (; ysteps > 0; ysteps--, pos.y++) {
		...
		for (int ix = 0; ix < m_size.x;) {
			...
			if (N == NORMAL || N == SCANLINE) {
				...
				uint8_t *sl2 =
						(N == BLEND) ? blend_line + pos.x + ix - bpos.x : sl;
				uint8_t *sl3 = datap;

				while (count--) {
					uint8_t *p1 = paddr + 3 * *sl2++;
					uint8_t *p2 = paddr
							+ 3 * (N == FADE_TINT ? tint[*sl3++] : *sl3++);
...
void TransImage::PutImage(image *screen, vec2i pos) {
	PutImageGeneric<NORMAL>(screen, pos, 0, NULL, 0, NULL, NULL, 0, 1, NULL,
			NULL, NULL);
}

void TransImage::PutRemap(image *screen, vec2i pos, uint8_t *map) {
	PutImageGeneric<REMAP>(screen, pos, 0, NULL, 0, map, NULL, 0, 1, NULL, NULL,
			NULL);
}

void TransImage::PutDoubleRemap(image *screen, vec2i pos, uint8_t *map,
		uint8_t *map2) {
	PutImageGeneric<REMAP2>(screen, pos, 0, NULL, 0, map, map2, 0, 1, NULL,
			NULL, NULL);
}
...
void TransImage::PutFade(image *screen, vec2i pos, int amount, int nframes,
	...
	PutImageGeneric<FADE>(screen, pos, 0, NULL, 0, NULL, NULL, amount, nframes,
			NULL, f, pal);
}

void TransImage::PutFadeTint(image *screen, vec2i pos, int amount, int nframes,
		uint8_t *tint, ColorFilter *f, palette *pal) {
	PutImageGeneric<FADE_TINT>(screen, pos, 0, NULL, 0, NULL, NULL, amount,
			nframes, tint, f, pal);
}

void TransImage::PutColor(image *screen, vec2i pos, uint8_t color) {
	PutImageGeneric<COLOR>(screen, pos, color, NULL, 0, NULL, NULL, 0, 1, NULL,
			NULL, NULL);
}
...
void TransImage::PutBlend(image *screen, vec2i pos, image *blend, vec2i bpos,
	...
	PutImageGeneric<BLEND>(screen, pos, 0, blend, bpos, NULL, NULL, amount, 1,
			NULL, f, pal);
}

void TransImage::PutFilled(image *screen, vec2i pos, uint8_t color) {
	PutImageGeneric<FILLED>(screen, pos, color, NULL, 0, NULL, NULL, 0, 1, NULL,
			NULL, NULL);
}

void TransImage::PutPredator(image *screen, vec2i pos) {
	PutImageGeneric<PREDATOR>(screen, pos, 0, NULL, 0, NULL, NULL, 0, 1, NULL,
			NULL, NULL);
}

void TransImage::PutScanLine(image *screen, vec2i pos, int line) {
	PutImageGeneric<SCANLINE>(screen, pos, 0, NULL, 0, NULL, NULL, line, 1,
			NULL, NULL, NULL);
}
