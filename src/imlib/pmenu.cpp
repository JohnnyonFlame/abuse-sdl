...
pmenu::pmenu(int X, int Y, pmenu_item *first, image *screen) {
	...
	bar->screen->widget_bar(0, 0, w - 1, h - 1, wm->bright_color(),
...
pmenu_item::pmenu_item(int ID, char const *Name, char const *on_off_flag,
		int Hotkey, pmenu_item *Next) {
	xp = -1;
	id = ID;
	hotkey = Hotkey;
	on_off = on_off_flag;
...
pmenu_item::pmenu_item(char const *Name, psub_menu *Sub, pmenu_item *Next,
		int xpos) {
	xp = xpos;
	id = 0;
	hotkey = -1;
...
void psub_menu::hide(Jwindow *parent, int x, int y) {
	...
	if (win) {
		if (active != -1) {
			int w, h;
			calc_size(w, h);
			item_num(active)->draw(win, x + 3,
					y + 3 + active * (wm->font()->height() + 1), w - 6, 0, 0);
		}
		wm->close_window(win);
		win = NULL;
	}
...
void psub_menu::draw(Jwindow *parent, int x, int y) {
	...
	win = wm->new_window(parent->x + x, parent->y + y,
			...
			h - Jwindow::top_border() - Jwindow::bottom_border(), NULL);
	win->freeze();
	win->screen->widget_bar(0, 0, w - 1, h - 1, wm->bright_color(),
			wm->medium_color(), wm->dark_color());
...
void pmenu_item::draw_self(Jwindow *parent, int x, int y, int w, int top,
		int active) {
	int bx = x;
	if (on_off)
		bx = x - wm->font()->width();
	...
	if (!n) {
		...
		parent->screen->widget_bar(x, y + h / 2 - 1, x + w - 1, y + h / 2,
				wm->dark_color(), wm->medium_color(), wm->bright_color());
	} else {
		if (active) {
			if (xp != -1)
				parent->screen->xor_bar(bx, y, x + w - 1,
						y + wm->font()->height() + 1, wm->dark_color());
			else {
				parent->screen->bar(bx, y, x + w - 1,
						y + wm->font()->height() + 1, wm->dark_color());
				wm->font()->put_string(parent->screen, x + 1, y + 1, n,
						wm->medium_color());
				if (on_off && *on_off)
					wm->font()->put_string(parent->screen, bx + 1, y + 1, "*",
							wm->medium_color());
			}
		} else {
			if (xp != -1)
				...
				parent->screen->bar(bx, y, x + w - 1,
						y + wm->font()->height() + 1, wm->medium_color());
				wm->font()->put_string(parent->screen, x + 1, y + 1, n,
						wm->bright_color());
				if (on_off && *on_off)
					wm->font()->put_string(parent->screen, bx + 1, y + 1, "*",
...
int psub_menu::handle_event(Jwindow *parent, int x, int y, event &ev) {
	...
	if (ev.mouse_move.x >= x && ev.mouse_move.y >= y && ev.mouse_move.x < x + w
		...
		if (new_active != active) {
			if (active != -1)
				item_num(active)->draw(win, dx, 3 + active * (th + 1), w - 6, 0,
						0);
			active = new_active;
			if (active != -1)
				item_num(active)->draw(win, dx, 3 + active * (th + 1), w - 6, 0,
		...
		if (ev.type == EV_MOUSE_BUTTON) {
			if (active != -1)
				return item_num(active)->handle_event(win, dx,
						3 + active * (th + 1), w - 6, 0, ev);
			else
				return 0;
		} else
			return 1;
	} else if (active != -1)
		return item_num(active)->handle_event(win, win->x + dx,
				win->y + 3 + active * (th + 1), w - 6, 0, ev);
	else
		return 0;
...
int pmenu_item::handle_event(Jwindow *parent, int x, int y, int w, int top,
	...
	if (ev.mouse_move.x >= x && ev.mouse_move.y >= y && ev.mouse_move.x < x + w
		...
		if (top)
			return sub->handle_event(parent, x, y + wm->font()->height() + 2,
					ev);
		else
...
pmenu_item *pmenu::inarea(int mx, int my, image *screen) {
	...
	mx -= bar->x;
	my -= bar->y;
	if (mx < 0 || my < 0 || mx >= bar->screen->Size().x
			|| my >= bar->screen->Size().y)
		return NULL;
	else {
...
int pmenu::handle_event(event &ev, image *screen) {
	...
	case EV_KEY: {
		...
		break;
	case EV_MOUSE_MOVE: {
		pmenu_item *new_selection = inarea(ev.mouse_move.x, ev.mouse_move.y,
				screen);
		if (!new_selection && active
				&& active->handle_event(bar, itemx(active), 1, itemw(active), 1,
						ev))
			return 1;
	...
	case EV_MOUSE_BUTTON: {
		if (active) {
			if (active->handle_event(bar, itemx(active), 1, itemw(active), 1,
					ev)) {
				active->draw(bar, itemx(active), 1, itemw(active), 1, 0);
				active = NULL;
				return 1;
