...
public:
	int t, sx;
	scroller(int X, int Y, int ID, int L, int H, int Vert, int Total_items,
			ifield *Next);
	...
	virtual void handle_inside_event(event &ev, image *screen,
			InputManager *inm) {
		;
	}
...
protected:
	...
	uint8_t *select;
public:
	spicker(int X, int Y, int ID, int Rows, int Cols, int Vert, int MultiSelect,
			ifield *Next);
	int vis() {
		if (vert)
			return r;
	...
	virtual void draw_item(image *screen, int x, int y, int num,
			int active) = 0;
	virtual int total() = 0;
...
class pick_list: public scroller {
	...
	image *tex;
public:
	pick_list(int X, int Y, int ID, int height, char **List, int num_entries,
			int start_yoffset, ifield *Next, image *texture = NULL);
	virtual void handle_inside_event(event &ev, image *screen,
			InputManager *inm);
	virtual void scroll_event(int newx, image *screen);
	virtual char *read() {
