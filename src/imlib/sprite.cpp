...
void sprite::restore_background() {
	if (x + save->Size().x >= 0 && y + save->Size().y >= 0 && x <= xres
			&& y <= yres)
		save->put_image(screen, x, y);
}

void sprite::get_background() {
	if (x + visual->Size().x >= 0 && y + visual->Size().y >= 0 && x <= xres
			&& y <= yres)
		screen->put_part(save, 0, 0, x, y, x + save->Size().x - 1,
				y + save->Size().y - 1);
}
...
void sprite_controller::remove_sprites() {
	sprite *sp;
	loopt(sprite, sp, sprites.first(), sp->restore_background(););
}

void sprite_controller::put_sprites() {
	sprite *sp;
	loopt(sprite, sp, sprites.first(), sp->draw(););
}

void sprite_controller::get_backgrounds() {
	sprite *sp;
	loopt(sprite, sp, sprites.first(), sp->get_background(););
}
