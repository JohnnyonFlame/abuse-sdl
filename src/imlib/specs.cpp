...
char const *spec_types[] = {
		...
		"Compress1 file", "Vector Image", "Light list", "Grue fgmap",
		"Grue bgmap", "Data array", "Character2", "Particle", "Extern lcache" };
...
int bFILE::write(void const *buf, size_t count) // returns number of bytes written
	...
	if (allow_write_buffering()) {
		...
		while (count) {
			int copy_size =
					wbuf_end + count <= wbuf_size ?
							count : wbuf_size - wbuf_end;
...
int jFILE::unbuffered_read(void *buf, size_t count) {
	...
	if (fd == spec_main_fd) {
		if (current_offset + start_offset != spec_main_offset)
			spec_main_offset = lseek(fd, start_offset + current_offset,
					SEEK_SET);
...
spec_entry::spec_entry(uint8_t spec_type, char const *object_name,
		char const *link_filename, unsigned long data_size,
...
void spec_directory::remove(spec_entry *e) {
	...
	if (entries[i] == e) // make sre it was found
		...
		entries = (spec_entry **) realloc(entries,
				sizeof(spec_entry *) * total);
	} else
