...
public:
	uint8_t keep_dirt, static_mem; // if set, don't free memory on exit
	...
	int bound_x1(int x1) {
		return x1 < m_clipx1 ? m_clipx1 : x1;
	}
	int bound_y1(int y1) {
		return y1 < m_clipy1 ? m_clipy1 : y1;
	}
	int bound_x2(int x2) {
		return x2 > m_clipx2 ? m_clipx2 : x2;
	}
	int bound_y2(int y2) {
		return y2 > m_clipy2 ? m_clipy2 : y2;
	}
	inline int x1_clip() {
		return m_clipx1;
	}
...
public:
	...
	void scroll(int16_t x1, int16_t y1, int16_t x2, int16_t y2, int16_t xd,
			int16_t yd);
	void fill_image(image *screen, int16_t x1, int16_t y1, int16_t x2,
			int16_t y2, int16_t align = 1);
	...
	void put_part_xrev(image *screen, int16_t x, int16_t y, int16_t x1,
			int16_t y1, int16_t x2, int16_t y2, char transparent = 0);
	...
	void AddDirty(int x1, int y1, int x2, int y2) {
		if (m_special)
			m_special->AddDirty(x1, y1, x2, y2);
	}
	void delete_dirty(int x1, int y1, int x2, int y2) {
		if (m_special)
			m_special->delete_dirty(x1, y1, x2, y2);
	}
	void ClearDirties() {
		if (m_special)
