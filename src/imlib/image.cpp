...
image_descriptor::image_descriptor(vec2i size, int keep_dirties,
		int static_memory) {
	m_clipx1 = 0;
	m_clipy1 = 0;
	m_l = size.x;
...
void image::PutPixel(vec2i pos, uint8_t color) {
	...
	if (m_special && pos.x >= m_special->x1_clip()
			&& pos.x < m_special->x2_clip() && pos.y >= m_special->y1_clip()
...
void image::line(int16_t x1, int16_t y1, int16_t x2, int16_t y2,
		uint8_t color) {
	...
	if (x1 < cx1 || x2 < cx1 || x1 >= cx2 || x2 >= cx2 || y1 < cy1 || y2 < cy1
			|| y1 >= cy2 || y2 >= cy2)
		return;
...
void image::put_image(image *screen, int16_t x, int16_t y, char transparent) {
	...
	if (x < screen->Size().x && y < screen->Size().y) {
		...
		for (j = starty; j < yl; j++, y++) {
			...
			pg2 = scan_line(j);
			if (transparent) {
				for (i = startx, source = pg2 + startx, dest = pg1 + x; i < xl;
						i++, source++, dest++) {
					if (*source)
						*dest = *source;
				}
...
void image::fill_image(image *screen, int16_t x1, int16_t y1, int16_t x2,
		int16_t y2, int16_t align) {
...
void image::put_part(image *screen, int16_t x, int16_t y, int16_t x1,
	...
	if (transparent) {
		for (j = 0; j < ylen; j++) {
			for (i = 0, source = &pg2[x1], dest = &pg1[x]; i < xlen;
					i++, source++, dest++)
				if (*source)
					*dest = *source;
...
void image::put_part_xrev(image *screen, int16_t x, int16_t y, int16_t x1,
		int16_t y1, int16_t x2, int16_t y2, char transparent) {
	int16_t xl, yl, j, i;
	uint8_t *pg1, *pg2, *source, *dest;
	CHECK(x1<=x2 && y1<=y2);
	...
	i = x1;
	x1 = m_size.x - x2 - 1; // reverse the x locations
	x2 = m_size.x - i - 1;
	...
	} else if (x > screen->Size().x || y > screen->Size().y || x + x2 < 0
			|| y + y2 < 0)
		return;

	if (x < screen->Size().x && y < screen->Size().y && x1 < m_size.x
			&& y1 < m_size.y && x1 <= x2 && y1 <= y2) {
		if (x2 >= m_size.x)
			x2 = m_size.x - 1;
		if (y2 >= m_size.y)
			y2 = m_size.y - 1;
		...
		for (j = 0; j < yl; j++) {
			...
			pg2 = scan_line(y1 + j);
			if (transparent) {
				for (i = 0, source = &pg2[x1], dest = &pg1[x + xl - 1]; i < xl;
						i++, source++, dest--)
					if (*source)
						*dest = *source;
			} else
...
void image::put_part_masked(image *screen, image *mask, int16_t x, int16_t y,
		int16_t maskx, int16_t masky, int16_t x1, int16_t y1, int16_t x2,
		int16_t y2) {
	...
	} else if (x > screen->Size().x || y > screen->Size().y || x + x1 < 0
			|| y + y1 < 0)
		return;
	...
	mh = mask->Size().y;
	if (x < screen->Size().x && y < screen->Size().y && x1 < m_size.x
			&& y1 < m_size.y && maskx < ml && masky < mh && x1 <= x2
			&& y1 <= y2) {
...
void image::rectangle(int16_t x1, int16_t y1, int16_t x2, int16_t y2,
		uint8_t color) {
	...
	for (p = (dirty_rect *) dirties.first(); i; i--, p = next) {
		...
		if (x2 >= p->dx2 + 1 && x1 <= p->dx1) {
			if (y2 >= p->dy2 + 1 && y1 <= p->dy1) {
				...
				dirties.add_front(
						new dirty_rect(p->dx1, p->dy1, p->dx2, y1 - 1));
				p->dy1 = y2;
			}
		}
		// does it take a y slice off (down)
			...
			if (x2 - 1 >= p->dx2)
				...
				dirties.add_front(
						new dirty_rect(p->dx1, p->dy1, x1 - 1, p->dy2));
				p->dx1 = x2;
			}
		}
			...
			if (x1 > p->dx1 && x2 - 1 < p->dx2) {
				if (y1 > p->dy1 && y2 - 1 < p->dy2) {
					dirties.add_front(
							new dirty_rect(p->dx1, p->dy1, p->dx2, y1 - 1));
					dirties.add_front(
							new dirty_rect(p->dx1, y2, p->dx2, p->dy2));
				} else if (y1 <= p->dy1)
					dirties.add_front(
							new dirty_rect(p->dx1, y2, p->dx2, p->dy2));
				else
...
void image_descriptor::AddDirty(int x1, int y1, int x2, int y2) {
	...
	if (!i)
		...
		for (p = (dirty_rect *) dirties.first(); i > 0; i--) {
			...
			if (x1 <= p->dx1 && y1 <= p->dy1 && x2 >= p->dx2 + 1
					&& y2 >= p->dy2 + 1) {
				dirty_rect *tmp = (dirty_rect*) p->Next();
				dirties.unlink(p);
				delete p;
				if (!dirties.first())
					...
					p = tmp;
			} else if (!(x2 - 1 < p->dx1 || y2 - 1 < p->dy1 || x1 > p->dx2
					|| y1 > p->dy2)) {
				...
				{
					if (x1 < p->dx1)
						AddDirty(x1, Max(y1, p->dy1), p->dx1,
								Min(y2, p->dy2 + 1));
					if (x2 > p->dx2 + 1)
						AddDirty(p->dx2 + 1, Max(y1, p->dy1), x2,
								Min(y2, p->dy2 + 1));
					if (y1 < p->dy1)
...
void image::bar(int16_t x1, int16_t y1, int16_t x2, int16_t y2, uint8_t color) {
	...
	if (x2 < 0 || y2 < 0 || x1 >= m_size.x || y1 >= m_size.y || x2 < x1
			|| y2 < y1)
		return;
	Lock();
	for (y = y1; y <= y2; y++)
...
void image::xor_bar(int16_t x1, int16_t y1, int16_t x2, int16_t y2,
		uint8_t color) {
	int16_t y, x;
	if (x1 > x2 || y1 > y2)
		return;
...
void image::dither(palette *pal) {
	int16_t x, y, i, j;
	uint8_t dt_matrix[] = { 0, 136, 24, 170, 68, 204, 102, 238, 51, 187, 17,
			153, 119, 255, 85, 221 };
...
void image::scroll(int16_t x1, int16_t y1, int16_t x2, int16_t y2, int16_t xd,
		int16_t yd) {
...
#define LED_L 5
#define LED_H 5
void image::burn_led(int16_t x, int16_t y, int32_t num, int16_t color,
		int16_t scale) {
	char st[100];
	int16_t ledx[] = { 1, 2, 1, 2, 3, 3, 3, 3, 1, 2, 0, 0, 0, 0 };
	int16_t ledy[] = { 3, 3, 0, 0, 1, 2, 4, 6, 7, 7, 4, 6, 1, 2 };

	int16_t dig[] = { 2 + 4 + 8 + 16 + 32 + 64, 4 + 8, 2 + 4 + 1 + 32 + 16, 2
			+ 4 + 1 + 8 + 16, 64 + 1 + 4 + 8, 2 + 64 + 1 + 8 + 16, 64 + 32 + 1
			+ 8 + 16, 2 + 4 + 8, 1 + 2 + 4 + 8 + 16 + 32 + 64, 64 + 2 + 4 + 1
			+ 8, 1 };
	int16_t xx, yy, zz;
	...
	for (xx = 0; xx < 8; xx++) {
		if (st[xx] != ' ') {
			...
			for (yy = 0; yy < 7; yy++)
				if ((1 << yy) & dig[zz])
					line(x + ledx[yy * 2] * scale, y + ledy[yy * 2] * scale,
							x + ledx[yy * 2 + 1] * scale,
...
uint8_t dither_matrix[] = { 0, 136, 24, 170, 68, 204, 102, 238, 51, 187, 17,
		153, 119, 255, 85, 221 };

image *image::copy_part_dithered(int16_t x1, int16_t y1, int16_t x2,
		int16_t y2) {
