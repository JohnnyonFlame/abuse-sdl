...
Filter::Filter(palette *from, palette *to) {
	...
	for (int i = 0; i < m_size; i++) {
		...
		if ((r || g || b) && to->red(color) == 0 && to->green(color) == 0
				&& to->blue(color) == 0)
			color = dk;
...
void Filter::PutImage(image *screen, image *im, vec2i pos) {
	int cx1, cy1, cx2, cy2, x1 = 0, y1 = 0, x2 = im->Size().x,
			y2 = im->Size().y;
	screen->GetClip(cx1, cy1, cx2, cy2);

	// See if the image gets clipped off the screen
	if (pos.x >= cx2 || pos.y >= cy2 || pos.x + (x2 - x1) <= cx1
			|| pos.y + (y2 - y1) <= cy1)
		return;
	...
	for (int r = 0; r < m_size; r++)
		for (int g = 0; g < m_size; g++)
			for (int b = 0; b < m_size; b++) {
				...
				for (int i = 0; i < max; i++) {
					int rd = *pp++ - r * mul, gd = *pp++ - g * mul, bd = *pp++
							- b * mul;
