...
public:
	...
	void clip_in() {
		screen->SetClip(x1(), y1(), x2() + 1, y2() + 1);
	}
	void clip_out() {
		screen->SetClip(0, 0, l, h);
	}
	char *read(int id) {
...
public:
	...
	Jwindow *new_window(int x, int y, int l, int h, ifield *fields = NULL,
			char const *Name = NULL);

	void set_frame_font(JCFont *fnt) {
		wframe_fnt = fnt;
	}
	JCFont *frame_font() {
		return wframe_fnt;
