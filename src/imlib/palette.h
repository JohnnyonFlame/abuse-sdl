...
public:
	...
	void get(int x, unsigned char &red, unsigned char &green,
			unsigned char &blue);
	...
	void load();
	void load_nice();
	void fade_to(int total_fades, int fade_on, int dest_r, int dest_g,
			int dest_b);
	...
	void set_all_used();
	void set_all_unused();
	void set_background(unsigned char b) {
		bg = b;
	}
	int background(unsigned char b) {
		return bg;
	}
...
public:
	...
	quant_node(int level, quant_node *dad, unsigned char r = 0,
