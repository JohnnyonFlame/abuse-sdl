...
public:
	...
	button(int X, int Y, int ID, image *Depressed, image *Pressed,
...
class button_box: public ifield {
	...
	int maxdown;
public:
	button_box(int X, int Y, int ID, int MaxDown, button *Buttons,
			ifield *Next);
...
class text_field: public ifield {
	...
	int xstart() {
		return x + wm->font()->width() * (strlen(prompt) + 1) + 3;
	}
	int xend() {
		return x + wm->font()->width() * (strlen(prompt) + 1 + strlen(format))
				+ 7;
	}
	int yend() {
		return y + wm->font()->height() + 5;
	}
	...
	void draw_text(image *screen) {
		screen->bar(xstart() + 1, y + 1, xend() - 1, yend() - 1,
				wm->dark_color());
